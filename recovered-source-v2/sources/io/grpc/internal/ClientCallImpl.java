...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
    ...
    private void startInternal(final ClientCall.Listener<RespT> listener, Metadata metadata) {
        ...
        if (effectiveDeadline == null || !effectiveDeadline.isExpired()) {
            ...
            this.stream = new FailingClientStream(Status.DEADLINE_EXCEEDED.withDescription(String.format("ClientCall started after %s deadline was exceeded %.9f seconds ago. Name resolution delay %.9f seconds.", str, Double.valueOf(timeRemaining / d), Double.valueOf(l == null ? 0.0d : l.longValue() / d))), clientStreamTracers);
        }
    ...
    private class DeadlineTimer implements Runnable {
        ...
        public void run() {
            ...
            sb.append("s. ");
            sb.append(String.format(Locale.US, "Name resolution delay %.9f seconds. ", Double.valueOf(((Long) ClientCallImpl.this.callOptions.getOption(ClientStreamTracer.NAME_RESOLUTION_DELAYED)) == null ? 0.0d : r1.longValue() / ClientCallImpl.NANO_TO_SECS)));
