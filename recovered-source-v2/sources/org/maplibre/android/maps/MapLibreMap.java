...
public final class MapLibreMap {
    ...
    MapLibreMap(NativeMap nativeMap, Transform transform, UiSettings uiSettings, Projection projection, OnGesturesManagerInteractionListener onGesturesManagerInteractionListener, CameraChangeDispatcher cameraChangeDispatcher, List<OnDeveloperAnimationListener> list) {
    ...
    public void setSwapBehaviorFlush(boolean z) {
        this.nativeMapView.setSwapBehaviorFlush(z);
    }

    void initialise(Context context, MapLibreMapOptions mapLibreMapOptions) {
        this.transform.initialise(this, mapLibreMapOptions);
        this.uiSettings.initialise(context, mapLibreMapOptions);
        setDebugActive(mapLibreMapOptions.getDebugActive());
        setApiBaseUrl(mapLibreMapOptions);
        setPrefetchesTiles(mapLibreMapOptions);
    }
    ...
    void onSaveInstanceState(Bundle bundle) {
        bundle.putParcelable(MapLibreConstants.STATE_CAMERA_POSITION, this.transform.getCameraPosition());
        bundle.putBoolean(MapLibreConstants.STATE_DEBUG_ACTIVE, isDebugActive());
        this.uiSettings.onSaveInstanceState(bundle);
    }

    void onRestoreInstanceState(Bundle bundle) {
        CameraPosition cameraPosition = (CameraPosition) bundle.getParcelable(MapLibreConstants.STATE_CAMERA_POSITION);
        ...
        this.nativeMapView.setDebug(bundle.getBoolean(MapLibreConstants.STATE_DEBUG_ACTIVE));
    }
    ...
    private void setPrefetchesTiles(MapLibreMapOptions mapLibreMapOptions) {
        if (!mapLibreMapOptions.getPrefetchesTiles()) {
            setPrefetchZoomDelta(0);
        } else {
            setPrefetchZoomDelta(mapLibreMapOptions.getPrefetchZoomDelta());
        }
    }
    ...
    public void setTileCacheEnabled(boolean z) {
        this.nativeMapView.setTileCacheEnabled(z);
    }
    ...
    public void setZoom(double d, PointF pointF, long j) {
        notifyDeveloperAnimationListeners();
        this.nativeMapView.setZoom(d, pointF, j);
    }
    ...
    private void setApiBaseUrl(MapLibreMapOptions mapLibreMapOptions) {
        String apiBaseUrl = mapLibreMapOptions.getApiBaseUrl();
