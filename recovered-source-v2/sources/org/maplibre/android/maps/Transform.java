...
public final class Transform implements MapView.OnCameraDidChangeListener {
    ...
    private final MapView.OnCameraDidChangeListener moveByChangeListener = new MapView.OnCameraDidChangeListener() { // from class: org.maplibre.android.maps.Transform.1
    ...
    void initialise(MapLibreMap mapLibreMap, MapLibreMapOptions mapLibreMapOptions) {
        CameraPosition camera = mapLibreMapOptions.getCamera();
        if (camera != null && !camera.equals(CameraPosition.DEFAULT)) {
            moveCamera(mapLibreMap, CameraUpdateFactory.newCameraPosition(camera), null);
        }
        setMinZoom(mapLibreMapOptions.getMinZoomPreference());
        setMaxZoom(mapLibreMapOptions.getMaxZoomPreference());
        setMinPitch(mapLibreMapOptions.getMinPitchPreference());
        setMaxPitch(mapLibreMapOptions.getMaxPitchPreference());
    }
    ...
    public void onCameraDidChange(boolean z) {
        if (z) {
            ...
            final MapLibreMap.CancelableCallback cancelableCallback = this.cameraCancelableCallback;
            if (cancelableCallback != null) {
                this.cameraCancelableCallback = null;
                this.handler.post(new Runnable() { // from class: org.maplibre.android.maps.Transform.2
    ...
    public final void moveCamera(MapLibreMap mapLibreMap, CameraUpdate cameraUpdate, final MapLibreMap.CancelableCallback cancelableCallback) {
        ...
        if (!isValidCameraPosition(cameraPosition)) {
            ...
            invalidateCameraPosition();
            this.cameraChangeDispatcher.onCameraIdle();
            this.handler.post(new Runnable() { // from class: org.maplibre.android.maps.Transform.3
                @Override // java.lang.Runnable
                public void run() {
                    MapLibreMap.CancelableCallback cancelableCallback2 = cancelableCallback;
    ...
    final void easeCamera(MapLibreMap mapLibreMap, CameraUpdate cameraUpdate, int i, boolean z, MapLibreMap.CancelableCallback cancelableCallback) {
        CameraPosition cameraPosition = cameraUpdate.getCameraPosition(mapLibreMap);
        if (!isValidCameraPosition(cameraPosition)) {
            if (cancelableCallback != null) {
                cancelableCallback.onFinish();
    ...
    public final void animateCamera(MapLibreMap mapLibreMap, CameraUpdate cameraUpdate, int i, MapLibreMap.CancelableCallback cancelableCallback) {
        CameraPosition cameraPosition = cameraUpdate.getCameraPosition(mapLibreMap);
        if (!isValidCameraPosition(cameraPosition)) {
            if (cancelableCallback != null) {
                cancelableCallback.onFinish();
    ...
    void cancelTransitions() {
        ...
        if (cancelableCallback != null) {
            this.cameraChangeDispatcher.onCameraIdle();
            this.cameraCancelableCallback = null;
            this.handler.post(new Runnable() { // from class: org.maplibre.android.maps.Transform.4
                @Override // java.lang.Runnable
                public void run() {
