...
class IconManager {
    ...
    private Icon loadDefaultIconForMarker(Marker marker) {
        Icon defaultMarker = IconFactory.getInstance(MapLibre.getApplicationContext()).defaultMarker();
    ...
    void ensureIconLoaded(Marker marker, MapLibreMap mapLibreMap) {
        Icon icon = marker.getIcon();
        if (icon == null) {
            icon = loadDefaultIconForMarker(marker);
        }
    ...
    private void setTopOffsetPixels(Marker marker, MapLibreMap mapLibreMap, Icon icon) {
        Marker marker2 = marker.getId() != -1 ? (Marker) mapLibreMap.getAnnotation(marker.getId()) : null;
