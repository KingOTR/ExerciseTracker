...
public class TileCoordinatesLayer extends Layer {
    ...
    private void drawTileCoordinates(TilePosition tilePosition, Canvas canvas) {
        ...
        if (this.drawSimple) {
            ...
            this.stringBuilder.append((int) tile.zoomLevel).append(" / ").append(tile.tileX).append(" / ").append(tile.tileY);
            String sb = this.stringBuilder.toString();
            int textWidth = (int) (tilePosition.point.x + ((tile.tileSize - this.paintBack.getTextWidth(sb)) / 2));
            int textHeight = (int) (tilePosition.point.y + ((tile.tileSize + this.paintBack.getTextHeight(sb)) / 2));
            canvas.drawText(sb, textWidth, textHeight, this.paintBack);
            canvas.drawText(sb, textWidth, textHeight, this.paintFront);
            return;
        }
        int scaleFactor = (int) (tilePosition.point.x + (this.displayModel.getScaleFactor() * 8.0f));
        int scaleFactor2 = (int) (tilePosition.point.y + (this.displayModel.getScaleFactor() * 24.0f));
        this.stringBuilder.setLength(0);
        ...
        float f = scaleFactor2;
        canvas.drawText(sb3, scaleFactor, (int) ((this.displayModel.getScaleFactor() * 24.0f) + f), this.paintBack);
        canvas.drawText(sb3, scaleFactor, (int) ((this.displayModel.getScaleFactor() * 24.0f) + f), this.paintFront);
        this.stringBuilder.setLength(0);
        this.stringBuilder.append("Z: ");
        this.stringBuilder.append((int) tile.zoomLevel);
        String sb4 = this.stringBuilder.toString();
        canvas.drawText(sb4, scaleFactor, (int) ((this.displayModel.getScaleFactor() * 48.0f) + f), this.paintBack);
        canvas.drawText(sb4, scaleFactor, (int) (f + (this.displayModel.getScaleFactor() * 48.0f)), this.paintFront);
    }
