...
public final class SharedElement {
    ...
    public final Rect getTargetBounds() {
        ...
        set_targetBounds(sharedElementInternalState != null ? RectKt.m3642Recttz77jQw(sharedElementInternalState.m109calculateLookaheadOffsetF1C5BW0(), sharedElementInternalState.m110getNonNullLookaheadSizeNHjbRc()) : null);
    ...
    public final void m108onLookaheadResultv_w8tDc(SharedElementInternalState state, long lookaheadSize, long topLeft) {
        if (state.getBoundsAnimation().getTarget()) {
            this.targetBoundsProvider = state;
            Rect rect = get_targetBounds();
            Offset m3591boximpl = rect != null ? Offset.m3591boximpl(rect.m3637getTopLeftF1C5BW0()) : null;
            if (m3591boximpl == null ? false : Offset.m3599equalsimpl0(m3591boximpl.getPackedValue(), topLeft)) {
                Rect rect2 = get_targetBounds();
                Size m3659boximpl = rect2 != null ? Size.m3659boximpl(rect2.m3635getSizeNHjbRc()) : null;
                if (m3659boximpl == null ? false : Size.m3667equalsimpl0(m3659boximpl.getPackedValue(), lookaheadSize)) {
                    return;
                }
            }
            Rect m3642Recttz77jQw = RectKt.m3642Recttz77jQw(topLeft, lookaheadSize);
            set_targetBounds(m3642Recttz77jQw);
            SnapshotStateList<SharedElementInternalState> snapshotStateList = this.states;
            int size = snapshotStateList.size();
            for (int i = 0; i < size; i++) {
                ...
                boundsAnimation.animate(currentBounds, m3642Recttz77jQw);
            }
        }
    }
