...
public abstract class NodeCoordinator extends LookaheadCapablePlaceable implements Measurable, LayoutCoordinates, OwnerScope {
    ...
    private static final float[] tmpMatrix = Matrix.m4074constructorimpl$default(null, 1, null);
    private static final HitTestSource PointerInputSource = new HitTestSource() { // from class: androidx.compose.ui.node.NodeCoordinator$Companion$PointerInputSource$1
        ...
        public void mo5409childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
            layoutNode.m5331hitTestM_7yMNQ$ui_release(pointerPosition, hitTestResult, isTouchEvent, isInLayer);
        }

        @Override // androidx.compose.ui.node.NodeCoordinator.HitTestSource
        /* renamed from: entityType-OLwlOKw, reason: not valid java name */
        public int mo5410entityTypeOLwlOKw() {
            return NodeKind.m5414constructorimpl(16);
        }
        ...
        public boolean interceptOutOfBoundsChildEvents(Modifier.Node node) {
            int m5414constructorimpl = NodeKind.m5414constructorimpl(16);
    ...
    private static final HitTestSource SemanticsSource = new HitTestSource() { // from class: androidx.compose.ui.node.NodeCoordinator$Companion$SemanticsSource$1
        ...
        public void mo5409childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
            layoutNode.m5332hitTestSemanticsM_7yMNQ$ui_release(pointerPosition, hitTestResult, isTouchEvent, isInLayer);
        }

        @Override // androidx.compose.ui.node.NodeCoordinator.HitTestSource
        /* renamed from: entityType-OLwlOKw */
        public int mo5410entityTypeOLwlOKw() {
            return NodeKind.m5414constructorimpl(8);
        }
    };
    private Density layerDensity = getLayoutNode().getDensity();
    private LayoutDirection layerLayoutDirection = getLayoutNode().getLayoutDirection();
    private float lastLayerAlpha = 0.8f;
    private long position = IntOffset.INSTANCE.m6441getZeronOccac();
    ...
    public interface HitTestSource {
        /* renamed from: childHitTest-YqVAtuI */
        void mo5409childHitTestYqVAtuI(LayoutNode layoutNode, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer);

        /* renamed from: entityType-OLwlOKw */
        int mo5410entityTypeOLwlOKw();
    ...
    public final /* synthetic */ <T> void m5407visitNodesaLcG6gQ(int type, Function1<? super T, Unit> block) {
        boolean m5423getIncludeSelfInTraversalH91voCI = NodeKindKt.m5423getIncludeSelfInTraversalH91voCI(type);
        Modifier.Node tail = getTail();
        if (!m5423getIncludeSelfInTraversalH91voCI && (tail = tail.getParent()) == null) {
            return;
        }
        for (Modifier.Node headNode = headNode(m5423getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet() & type) != 0; headNode = headNode.getChild()) {
    ...
    private final boolean m5386hasNodeH91voCI(int type) {
        Modifier.Node headNode = headNode(NodeKindKt.m5423getIncludeSelfInTraversalH91voCI(type));
        return headNode != null && DelegatableNodeKt.m5299has64DMado(headNode, type);
    }

    /* renamed from: head-H91voCI, reason: not valid java name */
    public final Modifier.Node m5400headH91voCI(int type) {
        boolean m5423getIncludeSelfInTraversalH91voCI = NodeKindKt.m5423getIncludeSelfInTraversalH91voCI(type);
        Modifier.Node tail = getTail();
        if (!m5423getIncludeSelfInTraversalH91voCI && (tail = tail.getParent()) == null) {
            return null;
    ...
    public final long mo5187getSizeYbymL2g() {
    ...
    public void replace$ui_release() {
        ...
        if (graphicsLayer != null) {
            mo5240placeAtf8xVGno(getPosition(), this.zIndex, graphicsLayer);
        } else {
            mo5180placeAtf8xVGno(getPosition(), this.zIndex, this.layerBlock);
        }
    }
    ...
    protected void onMeasureResultChanged(int width, int height) {
        ...
        if (ownedLayer != null) {
            ownedLayer.mo5465resizeozmzZPI(IntSizeKt.IntSize(width, height));
        } else if (getLayoutNode().isPlaced() && (nodeCoordinator = this.wrappedBy) != null) {
            nodeCoordinator.invalidateLayer();
        }
        m5241setMeasuredSizeozmzZPI(IntSizeKt.IntSize(width, height));
        if (this.layerBlock != null) {
            updateLayerParameters(false);
        }
        int m5414constructorimpl = NodeKind.m5414constructorimpl(4);
        boolean m5423getIncludeSelfInTraversalH91voCI = NodeKindKt.m5423getIncludeSelfInTraversalH91voCI(m5414constructorimpl);
        Modifier.Node tail = getTail();
        if (m5423getIncludeSelfInTraversalH91voCI || (tail = tail.getParent()) != null) {
            for (Modifier.Node headNode = headNode(m5423getIncludeSelfInTraversalH91voCI); headNode != null && (headNode.getAggregateChildKindSet() & m5414constructorimpl) != 0; headNode = headNode.getChild()) {
                if ((headNode.getKindSet() & m5414constructorimpl) != 0) {
    ...
    protected void m5405setPositiongyyYBs(long j) {
        this.position = j;
    }
    ...
    public Object getParentData() {
        if (!getLayoutNode().getNodes().m5375hasH91voCI$ui_release(NodeKind.m5414constructorimpl(64))) {
            return null;
        }
        getTail();
        Ref.ObjectRef objectRef = new Ref.ObjectRef();
        for (Modifier.Node tail = getLayoutNode().getNodes().getTail(); tail != null; tail = tail.getParent()) {
            if ((NodeKind.m5414constructorimpl(64) & tail.getKindSet()) != 0) {
                int m5414constructorimpl = NodeKind.m5414constructorimpl(64);
                MutableVector mutableVector = null;
    ...
    public final long m5398getLastMeasurementConstraintsmsEJaDk$ui_release() {
        return getMeasurementConstraints();
    }

    /* renamed from: performingMeasure-K40F9xA, reason: not valid java name */
    protected final Placeable m5403performingMeasureK40F9xA(long constraints, Function0<? extends Placeable> block) {
        m5242setMeasurementConstraintsBRTryo0(constraints);
        return block.invoke();
    }

    @Override // androidx.compose.ui.layout.Placeable
    /* renamed from: placeAt-f8xVGno */
    protected void mo5180placeAtf8xVGno(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock) {
        if (this.forcePlaceWithLookaheadOffset) {
            LookaheadDelegate lookaheadDelegate = getLookaheadDelegate();
            Intrinsics.checkNotNull(lookaheadDelegate);
            m5390placeSelfMLgxB_4(lookaheadDelegate.getPosition(), zIndex, layerBlock, null);
            return;
        }
        m5390placeSelfMLgxB_4(position, zIndex, layerBlock, null);
    }

    @Override // androidx.compose.ui.layout.Placeable
    /* renamed from: placeAt-f8xVGno */
    protected void mo5240placeAtf8xVGno(long position, float zIndex, GraphicsLayer layer) {
        if (this.forcePlaceWithLookaheadOffset) {
            LookaheadDelegate lookaheadDelegate = getLookaheadDelegate();
            Intrinsics.checkNotNull(lookaheadDelegate);
            m5390placeSelfMLgxB_4(lookaheadDelegate.getPosition(), zIndex, null, layer);
            return;
        }
        m5390placeSelfMLgxB_4(position, zIndex, null, layer);
    }

    /* renamed from: placeSelf-MLgxB_4, reason: not valid java name */
    private final void m5390placeSelfMLgxB_4(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock, GraphicsLayer explicitLayer) {
        if (explicitLayer != null) {
            ...
            if (this.layer == null) {
                ...
                createLayer.mo5465resizeozmzZPI(getMeasuredSize());
                createLayer.mo5464movegyyYBs(position);
        ...
        if (!IntOffset.m6430equalsimpl0(getPosition(), position)) {
            m5405setPositiongyyYBs(position);
            getLayoutNode().getLayoutDelegate().getMeasurePassDelegate().notifyChildrenUsingCoordinatesWhilePlacing();
            OwnedLayer ownedLayer = this.layer;
            if (ownedLayer != null) {
                ownedLayer.mo5464movegyyYBs(position);
            } else {
                NodeCoordinator nodeCoordinator = this.wrappedBy;
                if (nodeCoordinator != null) {
                    nodeCoordinator.invalidateLayer();
    ...
    public final void m5404placeSelfApparentToRealOffsetMLgxB_4(long position, float zIndex, Function1<? super GraphicsLayerScope, Unit> layerBlock, GraphicsLayer layer) {
        m5390placeSelfMLgxB_4(IntOffset.m6435plusqkQi6aY(position, getApparentToRealOffset()), zIndex, layerBlock, layer);
    }
    ...
    public final void draw(Canvas canvas, GraphicsLayer graphicsLayer) {
        ...
        float m6431getXimpl = IntOffset.m6431getXimpl(getPosition());
        float m6432getYimpl = IntOffset.m6432getYimpl(getPosition());
        canvas.translate(m6431getXimpl, m6432getYimpl);
    ...
    public final void updateLayerBlock(Function1<? super GraphicsLayerScope, Unit> layerBlock, boolean forceUpdateLayerParameters) {
        ...
        if (layoutNode.isAttached() && layerBlock != null) {
            ...
            createLayer$default.mo5465resizeozmzZPI(getMeasuredSize());
            createLayer$default.mo5464movegyyYBs(getPosition());
    ...
    private final void updateLayerParameters(boolean invokeOnLayoutChange) {
        ...
        if (ownedLayer != null) {
            ...
            if (function1 != null) {
                ...
                reusableGraphicsLayerScope.m4155setSizeuvyYCjk(IntSizeKt.m6485toSizeozmzZPI(mo5187getSizeYbymL2g()));
    ...
    public final long m5399getMinimumTouchTargetSizeNHjbRc() {
        return this.layerDensity.mo377toSizeXkaWNTQ(getLayoutNode().getViewConfiguration().mo5335getMinimumTouchTargetSizeMYxV2XQ());
    }

    /* renamed from: hitTest-YqVAtuI, reason: not valid java name */
    public final void m5401hitTestYqVAtuI(HitTestSource hitTestSource, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
        Modifier.Node m5400headH91voCI = m5400headH91voCI(hitTestSource.mo5410entityTypeOLwlOKw());
        if (!m5408withinLayerBoundsk4lQ0M(pointerPosition)) {
            if (isTouchEvent) {
                float m5396distanceInMinimumTouchTargettz77jQw = m5396distanceInMinimumTouchTargettz77jQw(pointerPosition, m5399getMinimumTouchTargetSizeNHjbRc());
                if (Float.isInfinite(m5396distanceInMinimumTouchTargettz77jQw) || Float.isNaN(m5396distanceInMinimumTouchTargettz77jQw) || !hitTestResult.isHitInMinimumTouchTargetBetter(m5396distanceInMinimumTouchTargettz77jQw, false)) {
                    return;
                }
                m5388hitNearJHbHoSQ(m5400headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, false, m5396distanceInMinimumTouchTargettz77jQw);
                return;
            }
            return;
        }
        if (m5400headH91voCI == null) {
            mo5323hitTestChildYqVAtuI(hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer);
            return;
        }
        if (m5402isPointerInBoundsk4lQ0M(pointerPosition)) {
            m5387hit1hIXUjU(m5400headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer);
            return;
        }
        float m5396distanceInMinimumTouchTargettz77jQw2 = !isTouchEvent ? Float.POSITIVE_INFINITY : m5396distanceInMinimumTouchTargettz77jQw(pointerPosition, m5399getMinimumTouchTargetSizeNHjbRc());
        if (!Float.isInfinite(m5396distanceInMinimumTouchTargettz77jQw2) && !Float.isNaN(m5396distanceInMinimumTouchTargettz77jQw2) && hitTestResult.isHitInMinimumTouchTargetBetter(m5396distanceInMinimumTouchTargettz77jQw2, isInLayer)) {
            m5388hitNearJHbHoSQ(m5400headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer, m5396distanceInMinimumTouchTargettz77jQw2);
        } else {
            m5391speculativeHitJHbHoSQ(m5400headH91voCI, hitTestSource, pointerPosition, hitTestResult, isTouchEvent, isInLayer, m5396distanceInMinimumTouchTargettz77jQw2);
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: hit-1hIXUjU, reason: not valid java name */
    public final void m5387hit1hIXUjU(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2) {
        if (node == null) {
            mo5323hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hit(node, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hit$1
                ...
                /* renamed from: invoke, reason: avoid collision after fix types in other method */
                public final void invoke2() {
                    Modifier.Node m5412nextUntilhw7D004;
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m5412nextUntilhw7D004 = NodeCoordinatorKt.m5412nextUntilhw7D004(node, hitTestSource.mo5410entityTypeOLwlOKw(), NodeKind.m5414constructorimpl(2));
                    nodeCoordinator.m5387hit1hIXUjU(m5412nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2);
                }
            });
        }
    }
    ...
    public final void m5388hitNearJHbHoSQ(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2, final float f) {
        if (node == null) {
            mo5323hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else {
            hitTestResult.hitInMinimumTouchTarget(node, f, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$hitNear$1
                ...
                public final void invoke2() {
                    ...
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m5412nextUntilhw7D004 = NodeCoordinatorKt.m5412nextUntilhw7D004(node, hitTestSource.mo5410entityTypeOLwlOKw(), NodeKind.m5414constructorimpl(2));
                    nodeCoordinator.m5388hitNearJHbHoSQ(m5412nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        }
    }
    ...
    public final void m5391speculativeHitJHbHoSQ(final Modifier.Node node, final HitTestSource hitTestSource, final long j, final HitTestResult hitTestResult, final boolean z, final boolean z2, final float f) {
        Modifier.Node m5412nextUntilhw7D004;
        if (node == null) {
            mo5323hitTestChildYqVAtuI(hitTestSource, j, hitTestResult, z, z2);
        } else if (!hitTestSource.interceptOutOfBoundsChildEvents(node)) {
            m5412nextUntilhw7D004 = NodeCoordinatorKt.m5412nextUntilhw7D004(node, hitTestSource.mo5410entityTypeOLwlOKw(), NodeKind.m5414constructorimpl(2));
            m5391speculativeHitJHbHoSQ(m5412nextUntilhw7D004, hitTestSource, j, hitTestResult, z, z2, f);
        } else {
            hitTestResult.speculativeHit(node, f, z2, new Function0<Unit>() { // from class: androidx.compose.ui.node.NodeCoordinator$speculativeHit$1
                ...
                /* renamed from: invoke, reason: avoid collision after fix types in other method */
                public final void invoke2() {
                    Modifier.Node m5412nextUntilhw7D0042;
                    NodeCoordinator nodeCoordinator = NodeCoordinator.this;
                    m5412nextUntilhw7D0042 = NodeCoordinatorKt.m5412nextUntilhw7D004(node, hitTestSource.mo5410entityTypeOLwlOKw(), NodeKind.m5414constructorimpl(2));
                    nodeCoordinator.m5391speculativeHitJHbHoSQ(m5412nextUntilhw7D0042, hitTestSource, j, hitTestResult, z, z2, f);
                }
            });
        }
    }
    ...
    public void mo5323hitTestChildYqVAtuI(HitTestSource hitTestSource, long pointerPosition, HitTestResult hitTestResult, boolean isTouchEvent, boolean isInLayer) {
        NodeCoordinator nodeCoordinator = this.wrapped;
        if (nodeCoordinator != null) {
            nodeCoordinator.m5401hitTestYqVAtuI(hitTestSource, m5385fromParentPosition8S9VItk$default(nodeCoordinator, pointerPosition, false, 2, null), hitTestResult, isTouchEvent, isInLayer);
        }
    }
    ...
    public final Rect touchBoundsInRoot() {
        ...
        long m5395calculateMinimumTouchTargetPaddingE7KxVPU = m5395calculateMinimumTouchTargetPaddingE7KxVPU(m5399getMinimumTouchTargetSizeNHjbRc());
        rectCache.setLeft(-Size.m3671getWidthimpl(m5395calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setTop(-Size.m3668getHeightimpl(m5395calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setRight(getMeasuredWidth() + Size.m3671getWidthimpl(m5395calculateMinimumTouchTargetPaddingE7KxVPU));
        rectCache.setBottom(getMeasuredHeight() + Size.m3668getHeightimpl(m5395calculateMinimumTouchTargetPaddingE7KxVPU));
    ...
    public long mo5193screenToLocalMKHz9U(long relativeToScreen) {
        if (!isAttached()) {
            InlineClassHelperKt.throwIllegalStateException(ExpectAttachedLayoutCoordinates);
        }
        return mo5188localPositionOfR5De75A(LayoutCoordinatesKt.findRootCoordinates(this), LayoutNodeKt.requireOwner(getLayoutNode()).mo5133screenToLocalMKHz9U(relativeToScreen));
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: localToScreen-MK-Hz9U */
    public long mo5191localToScreenMKHz9U(long relativeToLocal) {
        if (!isAttached()) {
            InlineClassHelperKt.throwIllegalStateException(ExpectAttachedLayoutCoordinates);
        }
        return LayoutNodeKt.requireOwner(getLayoutNode()).mo5132localToScreenMKHz9U(mo5190localToRootMKHz9U(relativeToLocal));
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: windowToLocal-MK-Hz9U */
    public long mo5196windowToLocalMKHz9U(long relativeToWindow) {
        if (!isAttached()) {
            InlineClassHelperKt.throwIllegalStateException(ExpectAttachedLayoutCoordinates);
        }
        LayoutCoordinates findRootCoordinates = LayoutCoordinatesKt.findRootCoordinates(this);
        return mo5188localPositionOfR5De75A(findRootCoordinates, Offset.m3606minusMKHz9U(LayoutNodeKt.requireOwner(getLayoutNode()).mo5467calculateLocalPositionMKHz9U(relativeToWindow), LayoutCoordinatesKt.positionInRoot(findRootCoordinates)));
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: localToWindow-MK-Hz9U */
    public long mo5192localToWindowMKHz9U(long relativeToLocal) {
        return LayoutNodeKt.requireOwner(getLayoutNode()).mo5468calculatePositionInWindowMKHz9U(mo5190localToRootMKHz9U(relativeToLocal));
    }
    ...
    public long mo5188localPositionOfR5De75A(LayoutCoordinates sourceCoordinates, long relativeToSource) {
        return mo5189localPositionOfS_NoaFU(sourceCoordinates, relativeToSource, true);
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: localPositionOf-S_NoaFU */
    public long mo5189localPositionOfS_NoaFU(LayoutCoordinates sourceCoordinates, long relativeToSource, boolean includeMotionFrameOfReference) {
        if (sourceCoordinates instanceof LookaheadLayoutCoordinates) {
            ((LookaheadLayoutCoordinates) sourceCoordinates).getCoordinator().onCoordinatesUsed$ui_release();
            return Offset.m3611unaryMinusF1C5BW0(sourceCoordinates.mo5189localPositionOfS_NoaFU(this, Offset.m3611unaryMinusF1C5BW0(relativeToSource), includeMotionFrameOfReference));
        }
        ...
        while (coordinator != findCommonAncestor$ui_release) {
            relativeToSource = coordinator.m5406toParentPosition8S9VItk(relativeToSource, includeMotionFrameOfReference);
            coordinator = coordinator.wrappedBy;
            Intrinsics.checkNotNull(coordinator);
        }
        return m5384ancestorToLocalS_NoaFU(findCommonAncestor$ui_release, relativeToSource, includeMotionFrameOfReference);
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: transformFrom-EL8BTi8 */
    public void mo5194transformFromEL8BTi8(LayoutCoordinates sourceCoordinates, float[] matrix) {
        NodeCoordinator coordinator = toCoordinator(sourceCoordinates);
        coordinator.onCoordinatesUsed$ui_release();
        NodeCoordinator findCommonAncestor$ui_release = findCommonAncestor$ui_release(coordinator);
        Matrix.m4083resetimpl(matrix);
        coordinator.m5394transformToAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
        m5393transformFromAncestorEL8BTi8(findCommonAncestor$ui_release, matrix);
    }

    @Override // androidx.compose.ui.layout.LayoutCoordinates
    /* renamed from: transformToScreen-58bKbWc */
    public void mo5195transformToScreen58bKbWc(float[] matrix) {
        Owner requireOwner = LayoutNodeKt.requireOwner(getLayoutNode());
        m5394transformToAncestorEL8BTi8(toCoordinator(LayoutCoordinatesKt.findRootCoordinates(this)), matrix);
        requireOwner.mo5131localToScreen58bKbWc(matrix);
    }

    /* renamed from: transformToAncestor-EL8BTi8, reason: not valid java name */
    private final void m5394transformToAncestorEL8BTi8(NodeCoordinator ancestor, float[] matrix) {
        while (!Intrinsics.areEqual(this, ancestor)) {
            OwnedLayer ownedLayer = this.layer;
            if (ownedLayer != null) {
                ownedLayer.mo5466transform58bKbWc(matrix);
            }
            if (!IntOffset.m6430equalsimpl0(this.getPosition(), IntOffset.INSTANCE.m6441getZeronOccac())) {
                float[] fArr = tmpMatrix;
                Matrix.m4083resetimpl(fArr);
                Matrix.m4094translateimpl$default(fArr, IntOffset.m6431getXimpl(r0), IntOffset.m6432getYimpl(r0), 0.0f, 4, null);
                Matrix.m4091timesAssign58bKbWc(matrix, fArr);
            }
            this = this.wrappedBy;
            Intrinsics.checkNotNull(this);
        }
    ...
    private final void m5393transformFromAncestorEL8BTi8(NodeCoordinator ancestor, float[] matrix) {
        if (Intrinsics.areEqual(ancestor, this)) {
            return;
        }
        NodeCoordinator nodeCoordinator = this.wrappedBy;
        Intrinsics.checkNotNull(nodeCoordinator);
        nodeCoordinator.m5393transformFromAncestorEL8BTi8(ancestor, matrix);
        if (!IntOffset.m6430equalsimpl0(getPosition(), IntOffset.INSTANCE.m6441getZeronOccac())) {
            float[] fArr = tmpMatrix;
            Matrix.m4083resetimpl(fArr);
            Matrix.m4094translateimpl$default(fArr, -IntOffset.m6431getXimpl(getPosition()), -IntOffset.m6432getYimpl(getPosition()), 0.0f, 4, null);
            Matrix.m4091timesAssign58bKbWc(matrix, fArr);
        }
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            ownedLayer.mo5461inverseTransform58bKbWc(matrix);
        }
    }
    ...
    public Rect localBoundingBoxOf(LayoutCoordinates sourceCoordinates, boolean clipBounds) {
        ...
        rectCache.setRight(IntSize.m6473getWidthimpl(sourceCoordinates.mo5187getSizeYbymL2g()));
        rectCache.setBottom(IntSize.m6472getHeightimpl(sourceCoordinates.mo5187getSizeYbymL2g()));
        while (coordinator != findCommonAncestor$ui_release) {
    ...
    private final long m5384ancestorToLocalS_NoaFU(NodeCoordinator ancestor, long offset, boolean includeMotionFrameOfReference) {
        if (ancestor == this) {
            return offset;
        }
        NodeCoordinator nodeCoordinator = this.wrappedBy;
        if (nodeCoordinator == null || Intrinsics.areEqual(ancestor, nodeCoordinator)) {
            return m5397fromParentPosition8S9VItk(offset, includeMotionFrameOfReference);
        }
        return m5397fromParentPosition8S9VItk(nodeCoordinator.m5384ancestorToLocalS_NoaFU(ancestor, offset, includeMotionFrameOfReference), includeMotionFrameOfReference);
    }
    ...
    public long mo5190localToRootMKHz9U(long relativeToLocal) {
        if (!isAttached()) {
            InlineClassHelperKt.throwIllegalStateException(ExpectAttachedLayoutCoordinates);
        }
        onCoordinatesUsed$ui_release();
        ...
        while (this != null) {
            j = m5392toParentPosition8S9VItk$default(this, j, false, 2, null);
            this = this.wrappedBy;
        }
        return j;
    }
    ...
    public static /* synthetic */ long m5392toParentPosition8S9VItk$default(NodeCoordinator nodeCoordinator, long j, boolean z, int i, Object obj) {
        if (obj != null) {
        ...
        return nodeCoordinator.m5406toParentPosition8S9VItk(j, z);
    }

    /* renamed from: toParentPosition-8S9VItk, reason: not valid java name */
    public long m5406toParentPosition8S9VItk(long position, boolean includeMotionFrameOfReference) {
        OwnedLayer ownedLayer = this.layer;
        if (ownedLayer != null) {
            position = ownedLayer.mo5463mapOffset8S9VItk(position, false);
        }
        return (includeMotionFrameOfReference || !getIsPlacedUnderMotionFrameOfReference()) ? IntOffsetKt.m6445plusNvtHpc(position, getPosition()) : position;
    }

    /* renamed from: fromParentPosition-8S9VItk$default, reason: not valid java name */
    public static /* synthetic */ long m5385fromParentPosition8S9VItk$default(NodeCoordinator nodeCoordinator, long j, boolean z, int i, Object obj) {
        if (obj != null) {
        ...
        return nodeCoordinator.m5397fromParentPosition8S9VItk(j, z);
    }

    /* renamed from: fromParentPosition-8S9VItk, reason: not valid java name */
    public long m5397fromParentPosition8S9VItk(long position, boolean includeMotionFrameOfReference) {
        if (includeMotionFrameOfReference || !getIsPlacedUnderMotionFrameOfReference()) {
            position = IntOffsetKt.m6443minusNvtHpc(position, getPosition());
        }
        OwnedLayer ownedLayer = this.layer;
        return ownedLayer != null ? ownedLayer.mo5463mapOffset8S9VItk(position, true) : position;
    }

    protected final void drawBorder(Canvas canvas, Paint paint) {
        canvas.drawRect(new Rect(0.5f, 0.5f, IntSize.m6473getWidthimpl(getMeasuredSize()) - 0.5f, IntSize.m6472getHeightimpl(getMeasuredSize()) - 0.5f), paint);
    }
    ...
    public final void rectInParent$ui_release(MutableRect bounds, boolean clipBounds, boolean clipToMinimumTouchTargetSize) {
        ...
        if (ownedLayer != null) {
            if (this.isClipping) {
                if (clipToMinimumTouchTargetSize) {
                    long m5399getMinimumTouchTargetSizeNHjbRc = m5399getMinimumTouchTargetSizeNHjbRc();
                    float m3671getWidthimpl = Size.m3671getWidthimpl(m5399getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    float m3668getHeightimpl = Size.m3668getHeightimpl(m5399getMinimumTouchTargetSizeNHjbRc) / 2.0f;
                    bounds.intersect(-m3671getWidthimpl, -m3668getHeightimpl, IntSize.m6473getWidthimpl(mo5187getSizeYbymL2g()) + m3671getWidthimpl, IntSize.m6472getHeightimpl(mo5187getSizeYbymL2g()) + m3668getHeightimpl);
                } else if (clipBounds) {
                    bounds.intersect(0.0f, 0.0f, IntSize.m6473getWidthimpl(mo5187getSizeYbymL2g()), IntSize.m6472getHeightimpl(mo5187getSizeYbymL2g()));
                }
                if (bounds.isEmpty()) {
                    return;
                }
        ...
        float m6431getXimpl = IntOffset.m6431getXimpl(getPosition());
        bounds.setLeft(bounds.getLeft() + m6431getXimpl);
        bounds.setRight(bounds.getRight() + m6431getXimpl);
        float m6432getYimpl = IntOffset.m6432getYimpl(getPosition());
        bounds.setTop(bounds.getTop() + m6432getYimpl);
        bounds.setBottom(bounds.getBottom() + m6432getYimpl);
    }
    ...
    protected final boolean m5408withinLayerBoundsk4lQ0M(long pointerPosition) {
        if (!OffsetKt.m3619isFinitek4lQ0M(pointerPosition)) {
            return false;
        }
        OwnedLayer ownedLayer = this.layer;
        return ownedLayer == null || !this.isClipping || ownedLayer.mo5462isInLayerk4lQ0M(pointerPosition);
    }

    /* renamed from: isPointerInBounds-k-4lQ0M, reason: not valid java name */
    protected final boolean m5402isPointerInBoundsk4lQ0M(long pointerPosition) {
        float m3602getXimpl = Offset.m3602getXimpl(pointerPosition);
        float m3603getYimpl = Offset.m3603getYimpl(pointerPosition);
        return m3602getXimpl >= 0.0f && m3603getYimpl >= 0.0f && m3602getXimpl < ((float) getMeasuredWidth()) && m3603getYimpl < ((float) getMeasuredHeight());
    }
    ...
    public final NodeCoordinator findCommonAncestor$ui_release(NodeCoordinator other) {
        ...
        if (layoutNode == layoutNode2) {
            ...
            int m5414constructorimpl = NodeKind.m5414constructorimpl(2);
            if (!tail2.getNode().getIsAttached()) {
                InlineClassHelperKt.throwIllegalStateException("visitLocalAncestors called on an unattached node");
            }
            for (Modifier.Node parent = tail2.getNode().getParent(); parent != null; parent = parent.getParent()) {
                if ((parent.getKindSet() & m5414constructorimpl) != 0 && parent == tail) {
                    return other;
                }
            }
            return this;
    ...
    private final long m5389offsetFromEdgeMKHz9U(long pointerPosition) {
        float m3602getXimpl = Offset.m3602getXimpl(pointerPosition);
        float max = Math.max(0.0f, m3602getXimpl < 0.0f ? -m3602getXimpl : m3602getXimpl - getMeasuredWidth());
        float m3603getYimpl = Offset.m3603getYimpl(pointerPosition);
        return OffsetKt.Offset(max, Math.max(0.0f, m3603getYimpl < 0.0f ? -m3603getYimpl : m3603getYimpl - getMeasuredHeight()));
    }

    /* renamed from: calculateMinimumTouchTargetPadding-E7KxVPU, reason: not valid java name */
    protected final long m5395calculateMinimumTouchTargetPaddingE7KxVPU(long minimumTouchTargetSize) {
        return SizeKt.Size(Math.max(0.0f, (Size.m3671getWidthimpl(minimumTouchTargetSize) - getMeasuredWidth()) / 2.0f), Math.max(0.0f, (Size.m3668getHeightimpl(minimumTouchTargetSize) - getMeasuredHeight()) / 2.0f));
    }

    /* renamed from: distanceInMinimumTouchTarget-tz77jQw, reason: not valid java name */
    protected final float m5396distanceInMinimumTouchTargettz77jQw(long pointerPosition, long minimumTouchTargetSize) {
        if (getMeasuredWidth() >= Size.m3671getWidthimpl(minimumTouchTargetSize) && getMeasuredHeight() >= Size.m3668getHeightimpl(minimumTouchTargetSize)) {
            return Float.POSITIVE_INFINITY;
        }
        long m5395calculateMinimumTouchTargetPaddingE7KxVPU = m5395calculateMinimumTouchTargetPaddingE7KxVPU(minimumTouchTargetSize);
        float m3671getWidthimpl = Size.m3671getWidthimpl(m5395calculateMinimumTouchTargetPaddingE7KxVPU);
        float m3668getHeightimpl = Size.m3668getHeightimpl(m5395calculateMinimumTouchTargetPaddingE7KxVPU);
        long m5389offsetFromEdgeMKHz9U = m5389offsetFromEdgeMKHz9U(pointerPosition);
        if ((m3671getWidthimpl > 0.0f || m3668getHeightimpl > 0.0f) && Offset.m3602getXimpl(m5389offsetFromEdgeMKHz9U) <= m3671getWidthimpl && Offset.m3603getYimpl(m5389offsetFromEdgeMKHz9U) <= m3668getHeightimpl) {
            return Offset.m3601getDistanceSquaredimpl(m5389offsetFromEdgeMKHz9U);
        }
        return Float.POSITIVE_INFINITY;
    }
    ...
    public final void onMeasured() {
        ...
        if (m5386hasNodeH91voCI(NodeKind.m5414constructorimpl(128))) {
            Snapshot.Companion companion = Snapshot.INSTANCE;
            Snapshot currentThreadSnapshot = companion.getCurrentThreadSnapshot();
            Function1<Object, Unit> readObserver = currentThreadSnapshot != null ? currentThreadSnapshot.getReadObserver() : null;
            Snapshot makeCurrentNonObservable = companion.makeCurrentNonObservable(currentThreadSnapshot);
            try {
                int m5414constructorimpl = NodeKind.m5414constructorimpl(128);
                boolean m5423getIncludeSelfInTraversalH91voCI = NodeKindKt.m5423getIncludeSelfInTraversalH91voCI(m5414constructorimpl);
                if (m5423getIncludeSelfInTraversalH91voCI) {
                    parent = getTail();
                } else {
                    ...
                    if ((headNode.getKindSet() & m5414constructorimpl) != 0) {
                        ...
                        while (delegatingNode != 0) {
                            if (delegatingNode instanceof LayoutAwareModifierNode) {
                                ((LayoutAwareModifierNode) delegatingNode).mo366onRemeasuredozmzZPI(getMeasuredSize());
                            } else if ((delegatingNode.getKindSet() & m5414constructorimpl) != 0 && (delegatingNode instanceof DelegatingNode)) {
                                Modifier.Node delegate = delegatingNode.getDelegate();
                                int i = 0;
                                delegatingNode = delegatingNode;
    ...
    public final void drawContainedDrawModifiers(Canvas canvas, GraphicsLayer graphicsLayer) {
        Modifier.Node m5400headH91voCI = m5400headH91voCI(NodeKind.m5414constructorimpl(4));
        if (m5400headH91voCI == null) {
            performDraw(canvas, graphicsLayer);
        } else {
            getLayoutNode().getMDrawScope$ui_release().m5341draweZhPAX0$ui_release(canvas, IntSizeKt.m6485toSizeozmzZPI(mo5187getSizeYbymL2g()), this, m5400headH91voCI, graphicsLayer);
        }
    }
    ...
    public final boolean shouldSharePointerInputWithSiblings() {
        Modifier.Node headNode = headNode(NodeKindKt.m5423getIncludeSelfInTraversalH91voCI(NodeKind.m5414constructorimpl(16)));
