...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long m5322findBestHitDistanceptXAw2c = m5322findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m5317getDistanceimpl(m5322findBestHitDistanceptXAw2c) < 0.0f && DistanceAndInLayer.m5319isInLayerimpl(m5322findBestHitDistanceptXAw2c);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        return DistanceAndInLayer.m5313compareToS_HNhKs(m5322findBestHitDistanceptXAw2c(), DistanceAndInLayer) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m5322findBestHitDistanceptXAw2c() {
        long DistanceAndInLayer;
        ...
        if (i <= lastIndex) {
            while (true) {
                long m5314constructorimpl = DistanceAndInLayer.m5314constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                if (DistanceAndInLayer.m5313compareToS_HNhKs(m5314constructorimpl, DistanceAndInLayer) < 0) {
                    DistanceAndInLayer = m5314constructorimpl;
                }
                if (DistanceAndInLayer.m5317getDistanceimpl(DistanceAndInLayer) < 0.0f && DistanceAndInLayer.m5319isInLayerimpl(DistanceAndInLayer)) {
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(hitTestResult) && DistanceAndInLayer.m5313compareToS_HNhKs(m5322findBestHitDistanceptXAw2c, m5322findBestHitDistanceptXAw2c()) > 0) {
