...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m3897toArgb8_81llA(colors.get(i).m3853unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                m3603getYimpl = Offset.m3602getXimpl(packedValue);
            } else {
                m3603getYimpl = Offset.m3603getYimpl(packedValue);
            }
            fArr[i] = m3603getYimpl;
        }
        return fArr;
    }
}
