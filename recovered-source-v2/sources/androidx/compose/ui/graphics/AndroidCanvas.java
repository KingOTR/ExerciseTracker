...
public final class AndroidCanvas implements Canvas {
    ...
    public void mo3698concat58bKbWc(float[] matrix) {
        if (MatrixKt.m4098isIdentity58bKbWc(matrix)) {
            return;
        }
        android.graphics.Matrix matrix2 = new android.graphics.Matrix();
        AndroidMatrixConversions_androidKt.m3713setFromEL8BTi8(matrix2, matrix);
        this.internalCanvas.concat(matrix2);
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipRect-N_I0leg, reason: not valid java name */
    public void mo3697clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m3706toRegionOp7u2Bmg(clipOp));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipPath-mtrdD-E, reason: not valid java name */
    public void mo3696clipPathmtrdDE(Path path, int clipOp) {
        android.graphics.Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m3706toRegionOp7u2Bmg(clipOp));
            return;
        }
    ...
    public final Region.Op m3706toRegionOp7u2Bmg(int i) {
        return ClipOp.m3827equalsimpl0(i, ClipOp.INSTANCE.m3831getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawLine-Wko1d7g, reason: not valid java name */
    public void mo3702drawLineWko1d7g(long p1, long p2, Paint paint) {
        this.internalCanvas.drawLine(Offset.m3602getXimpl(p1), Offset.m3603getYimpl(p1), Offset.m3602getXimpl(p2), Offset.m3603getYimpl(p2), paint.getInternalPaint());
    }
    ...
    public void mo3699drawCircle9KIMszo(long center, float radius, Paint paint) {
        this.internalCanvas.drawCircle(Offset.m3602getXimpl(center), Offset.m3603getYimpl(center), radius, paint.getInternalPaint());
    }
    ...
    public void mo3700drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(image), Offset.m3602getXimpl(topLeftOffset), Offset.m3603getYimpl(topLeftOffset), paint.getInternalPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawImageRect-HPBpro0, reason: not valid java name */
    public void mo3701drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        ...
        rect.left = IntOffset.m6431getXimpl(srcOffset);
        rect.top = IntOffset.m6432getYimpl(srcOffset);
        rect.right = IntOffset.m6431getXimpl(srcOffset) + IntSize.m6473getWidthimpl(srcSize);
        rect.bottom = IntOffset.m6432getYimpl(srcOffset) + IntSize.m6472getHeightimpl(srcSize);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Intrinsics.checkNotNull(rect2);
        rect2.left = IntOffset.m6431getXimpl(dstOffset);
        rect2.top = IntOffset.m6432getYimpl(dstOffset);
        rect2.right = IntOffset.m6431getXimpl(dstOffset) + IntSize.m6473getWidthimpl(dstSize);
        rect2.bottom = IntOffset.m6432getYimpl(dstOffset) + IntSize.m6472getHeightimpl(dstSize);
    ...
    public void mo3703drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        if (PointMode.m4144equalsimpl0(pointMode, PointMode.INSTANCE.m4148getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m4144equalsimpl0(pointMode, PointMode.INSTANCE.m4150getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m4144equalsimpl0(pointMode, PointMode.INSTANCE.m4149getPointsr_lszbg())) {
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            ...
            while (i < points.size() - 1) {
                ...
                this.internalCanvas.drawLine(Offset.m3602getXimpl(packedValue), Offset.m3603getYimpl(packedValue), Offset.m3602getXimpl(packedValue2), Offset.m3603getYimpl(packedValue2), internalPaint);
                i += stepBy;
            }
        }
    }
    ...
    public void mo3704drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
    ...
    public void mo3705drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m3749toAndroidVertexModeJOOmi9M(vertices.getVertexMode()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.getInternalPaint());
    }
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int i = 0; i < size; i++) {
            long packedValue = points.get(i).getPackedValue();
            this.internalCanvas.drawPoint(Offset.m3602getXimpl(packedValue), Offset.m3603getYimpl(packedValue), paint.getInternalPaint());
        }
    }
}
