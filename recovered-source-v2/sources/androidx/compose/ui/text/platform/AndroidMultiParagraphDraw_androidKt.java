...
public final class AndroidMultiParagraphDraw_androidKt {
    ...
    public static final void m6042drawMultiParagraph7AXcY_I(MultiParagraph multiParagraph, Canvas canvas, Brush brush, float f, Shadow shadow, TextDecoration textDecoration, DrawStyle drawStyle, int i) {
        canvas.save();
        if (multiParagraph.getParagraphInfoList$ui_text_release().size() <= 1) {
            m6044drawParagraphs7AXcY_I(multiParagraph, canvas, brush, f, shadow, textDecoration, drawStyle, i);
            ...
            Shader mo3812createShaderuvyYCjk = ((ShaderBrush) brush).mo3812createShaderuvyYCjk(SizeKt.Size(f2, f3));
            Matrix matrix = new Matrix();
            mo3812createShaderuvyYCjk.getLocalMatrix(matrix);
            ...
            for (int i3 = 0; i3 < size2; i3++) {
                ...
                paragraphInfo2.getParagraph().mo5626painthn5TExg(canvas, BrushKt.ShaderBrush(mo3812createShaderuvyYCjk), f, shadow, textDecoration, drawStyle, i);
                ...
                mo3812createShaderuvyYCjk.setLocalMatrix(matrix);
            }
        }
        canvas.restore();
    }
    ...
    private static final void m6044drawParagraphs7AXcY_I(MultiParagraph multiParagraph, Canvas canvas, Brush brush, float f, Shadow shadow, TextDecoration textDecoration, DrawStyle drawStyle, int i) {
        List<ParagraphInfo> paragraphInfoList$ui_text_release = multiParagraph.getParagraphInfoList$ui_text_release();
        int size = paragraphInfoList$ui_text_release.size();
        for (int i2 = 0; i2 < size; i2++) {
            ParagraphInfo paragraphInfo = paragraphInfoList$ui_text_release.get(i2);
            paragraphInfo.getParagraph().mo5626painthn5TExg(canvas, brush, f, shadow, textDecoration, drawStyle, i);
