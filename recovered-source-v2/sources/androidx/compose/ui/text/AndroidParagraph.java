...
public final class AndroidParagraph implements Paragraph {
    ...
    private AndroidParagraph(AndroidParagraphIntrinsics androidParagraphIntrinsics, int i, boolean z, long j) {
        ...
        int m5633toLayoutAlignaXe7zB0;
        int m5635toLayoutHyphenationFrequency3fSNIE;
        int m5634toLayoutBreakStrategyxImikfE;
        int m5636toLayoutLineBreakStylehpcqdu8;
        int m5637toLayoutLineBreakWordStylewPN0Rpw;
        ArrayList arrayList;
        Rect rect;
        ...
        this.ellipsis = z;
        this.constraints = j;
        if (Constraints.m6257getMinHeightimpl(j) != 0 || Constraints.m6258getMinWidthimpl(j) != 0) {
        ...
        m5633toLayoutAlignaXe7zB0 = AndroidParagraph_androidKt.m5633toLayoutAlignaXe7zB0(style.m5829getTextAligne0LSkKk());
        boolean m6188equalsimpl0 = TextAlign.m6188equalsimpl0(style.m5829getTextAligne0LSkKk(), TextAlign.INSTANCE.m6194getJustifye0LSkKk());
        m5635toLayoutHyphenationFrequency3fSNIE = AndroidParagraph_androidKt.m5635toLayoutHyphenationFrequency3fSNIE(style.getParagraphStyle().getHyphens());
        m5634toLayoutBreakStrategyxImikfE = AndroidParagraph_androidKt.m5634toLayoutBreakStrategyxImikfE(LineBreak.m6112getStrategyfcGXIks(style.m5826getLineBreakrAG3T2k()));
        m5636toLayoutLineBreakStylehpcqdu8 = AndroidParagraph_androidKt.m5636toLayoutLineBreakStylehpcqdu8(LineBreak.m6113getStrictnessusljTpc(style.m5826getLineBreakrAG3T2k()));
        m5637toLayoutLineBreakWordStylewPN0Rpw = AndroidParagraph_androidKt.m5637toLayoutLineBreakWordStylewPN0Rpw(LineBreak.m6114getWordBreakjp8hJ3c(style.m5826getLineBreakrAG3T2k()));
        TextUtils.TruncateAt truncateAt = z ? TextUtils.TruncateAt.END : null;
        TextLayout constructTextLayout = constructTextLayout(m5633toLayoutAlignaXe7zB0, m6188equalsimpl0 ? 1 : 0, truncateAt, i, m5635toLayoutHyphenationFrequency3fSNIE, m5634toLayoutBreakStrategyxImikfE, m5636toLayoutLineBreakStylehpcqdu8, m5637toLayoutLineBreakWordStylewPN0Rpw);
        if (z && constructTextLayout.getHeight() > Constraints.m6255getMaxHeightimpl(j) && i > 1) {
            numberOfLinesThatFitMaxHeight = AndroidParagraph_androidKt.numberOfLinesThatFitMaxHeight(constructTextLayout, Constraints.m6255getMaxHeightimpl(j));
            if (numberOfLinesThatFitMaxHeight >= 0 && numberOfLinesThatFitMaxHeight != i) {
                constructTextLayout = constructTextLayout(m5633toLayoutAlignaXe7zB0, m6188equalsimpl0 ? 1 : 0, truncateAt, RangesKt.coerceAtLeast(numberOfLinesThatFitMaxHeight, 1), m5635toLayoutHyphenationFrequency3fSNIE, m5634toLayoutBreakStrategyxImikfE, m5636toLayoutLineBreakStylehpcqdu8, m5637toLayoutLineBreakWordStylewPN0Rpw);
            }
            this.layout = constructTextLayout;
        } else {
            this.layout = constructTextLayout;
        }
        getTextPaint$ui_text_release().m6055setBrush12SF9DM(style.getBrush(), SizeKt.Size(getWidth(), getHeight()), style.getAlpha());
        ...
        if (shaderBrushSpans != null) {
            ...
            while (it.hasNext()) {
                ((ShaderBrushSpan) it.next()).m6075setSizeuvyYCjk(SizeKt.Size(getWidth(), getHeight()));
            }
        }
    ...
    public float getWidth() {
        return Constraints.m6256getMaxWidthimpl(this.constraints);
    }
    ...
    public int mo5621getOffsetForPositionk4lQ0M(long position) {
        return this.layout.getOffsetForHorizontal(this.layout.getLineForVertical((int) Offset.m3603getYimpl(position)), Offset.m3602getXimpl(position));
    }

    @Override // androidx.compose.ui.text.Paragraph
    /* renamed from: getRangeForRect-8-6BmAI, reason: not valid java name */
    public long mo5622getRangeForRect86BmAI(Rect rect, int granularity, final TextInclusionStrategy inclusionStrategy) {
        int m5638toLayoutTextGranularityduNsdkg;
        TextLayout textLayout = this.layout;
        RectF androidRectF = RectHelper_androidKt.toAndroidRectF(rect);
        m5638toLayoutTextGranularityduNsdkg = AndroidParagraph_androidKt.m5638toLayoutTextGranularityduNsdkg(granularity);
        int[] rangeForRect = textLayout.getRangeForRect(androidRectF, m5638toLayoutTextGranularityduNsdkg, new Function2<RectF, RectF, Boolean>() { // from class: androidx.compose.ui.text.AndroidParagraph$getRangeForRect$range$1
            {
                super(2);
            }
    ...
    public void mo5619fillBoundingBoxes8ffj60Q(long range, float[] array, int arrayStart) {
        this.layout.fillBoundingBoxes(TextRange.m5786getMinimpl(range), TextRange.m5785getMaximpl(range), array, arrayStart);
    }
    ...
    public long mo5623getWordBoundaryjx7JFs(int offset) {
    ...
    public void mo5625paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration textDecoration) {
        AndroidTextPaint textPaint$ui_text_release = getTextPaint$ui_text_release();
        textPaint$ui_text_release.m6057setColor8_81llA(color);
    ...
    public void mo5624paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration textDecoration, DrawStyle drawStyle, int blendMode) {
        ...
        textPaint$ui_text_release.m6054setBlendModes9anfk8(blendMode);
        paint(canvas);
        getTextPaint$ui_text_release().m6054setBlendModes9anfk8(backingBlendMode);
    }

    @Override // androidx.compose.ui.text.Paragraph
    /* renamed from: paint-hn5TExg, reason: not valid java name */
    public void mo5626painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration textDecoration, DrawStyle drawStyle, int blendMode) {
        int backingBlendMode = getTextPaint$ui_text_release().getBackingBlendMode();
        AndroidTextPaint textPaint$ui_text_release = getTextPaint$ui_text_release();
        textPaint$ui_text_release.m6055setBrush12SF9DM(brush, SizeKt.Size(getWidth(), getHeight()), alpha);
        textPaint$ui_text_release.setShadow(shadow);
        textPaint$ui_text_release.setTextDecoration(textDecoration);
        textPaint$ui_text_release.setDrawStyle(drawStyle);
        textPaint$ui_text_release.m6054setBlendModes9anfk8(blendMode);
