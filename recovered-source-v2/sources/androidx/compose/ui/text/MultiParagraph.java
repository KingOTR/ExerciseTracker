...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics multiParagraphIntrinsics, long j, int i, boolean z) {
        ...
        this.maxLines = i;
        if (Constraints.m6258getMinWidthimpl(j) != 0 || Constraints.m6257getMinHeightimpl(j) != 0) {
        ...
        while (i4 < size) {
            ...
            if (Constraints.m6251getHasBoundedHeightimpl(j)) {
                m6255getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m6255getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                m6255getMaxHeightimpl = Constraints.m6255getMaxHeightimpl(j);
            }
            Paragraph m5672Paragraph_EkL_Y = ParagraphKt.m5672Paragraph_EkL_Y(intrinsics, ConstraintsKt.Constraints$default(0, m6256getMaxWidthimpl, 0, m6255getMaxHeightimpl, 5, null), this.maxLines - i3, z);
            float height = f + m5672Paragraph_EkL_Y.getHeight();
            int lineCount = i3 + m5672Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(m5672Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i3, lineCount, f, height));
            if (m5672Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i4 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i3 = lineCount;
                f = height;
                break;
        ...
        this.width = Constraints.m6256getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m5651paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m3879getUnspecified0d7_KjU();
        }
        multiParagraph.m5658paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }
    ...
    public final /* synthetic */ void m5658paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo5625paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m5657paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo5624paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m5659painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        AndroidMultiParagraphDraw_androidKt.m6042drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(final int start, final int end) {
        ...
        MultiParagraphKt.m5660findParagraphsByRangeSbBc2M(this.paragraphInfoList, TextRangeKt.TextRange(start, end), new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph$getPathForRange$2
            ...
            public final void invoke2(ParagraphInfo paragraphInfo) {
                Path.m4113addPathUv8p0NA$default(Path.this, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
            }
        });
        return Path;
    }
    ...
    public final int m5654getOffsetForPositionk4lQ0M(long position) {
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m3603getYimpl(position)));
        if (paragraphInfo.getLength() == 0) {
            return paragraphInfo.getStartIndex();
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo5621getOffsetForPositionk4lQ0M(paragraphInfo.m5669toLocalMKHz9U(position)));
    }

    /* renamed from: getRangeForRect-8-6BmAI, reason: not valid java name */
    public final long m5655getRangeForRect86BmAI(Rect rect, int granularity, TextInclusionStrategy inclusionStrategy) {
        ...
        if (this.paragraphInfoList.get(findParagraphByY).getBottom() >= rect.getBottom() || findParagraphByY == CollectionsKt.getLastIndex(this.paragraphInfoList)) {
            ...
            return ParagraphInfo.m5667toGlobalxdX6G0$default(paragraphInfo, paragraphInfo.getParagraph().mo5622getRangeForRect86BmAI(paragraphInfo.toLocal(rect), granularity, inclusionStrategy), false, 1, null);
        }
        int findParagraphByY2 = MultiParagraphKt.findParagraphByY(this.paragraphInfoList, rect.getBottom());
        long m5793getZerod9O1mEE = TextRange.INSTANCE.m5793getZerod9O1mEE();
        while (TextRange.m5781equalsimpl0(m5793getZerod9O1mEE, TextRange.INSTANCE.m5793getZerod9O1mEE()) && findParagraphByY <= findParagraphByY2) {
            ParagraphInfo paragraphInfo2 = this.paragraphInfoList.get(findParagraphByY);
            m5793getZerod9O1mEE = ParagraphInfo.m5667toGlobalxdX6G0$default(paragraphInfo2, paragraphInfo2.getParagraph().mo5622getRangeForRect86BmAI(paragraphInfo2.toLocal(rect), granularity, inclusionStrategy), false, 1, null);
            findParagraphByY++;
        }
        if (TextRange.m5781equalsimpl0(m5793getZerod9O1mEE, TextRange.INSTANCE.m5793getZerod9O1mEE())) {
            return TextRange.INSTANCE.m5793getZerod9O1mEE();
        }
        long m5793getZerod9O1mEE2 = TextRange.INSTANCE.m5793getZerod9O1mEE();
        while (TextRange.m5781equalsimpl0(m5793getZerod9O1mEE2, TextRange.INSTANCE.m5793getZerod9O1mEE()) && findParagraphByY <= findParagraphByY2) {
            ParagraphInfo paragraphInfo3 = this.paragraphInfoList.get(findParagraphByY2);
            m5793getZerod9O1mEE2 = ParagraphInfo.m5667toGlobalxdX6G0$default(paragraphInfo3, paragraphInfo3.getParagraph().mo5622getRangeForRect86BmAI(paragraphInfo3.toLocal(rect), granularity, inclusionStrategy), false, 1, null);
            findParagraphByY2--;
        }
        return TextRange.m5781equalsimpl0(m5793getZerod9O1mEE2, TextRange.INSTANCE.m5793getZerod9O1mEE()) ? m5793getZerod9O1mEE : TextRangeKt.TextRange(TextRange.m5788getStartimpl(m5793getZerod9O1mEE), TextRange.m5783getEndimpl(m5793getZerod9O1mEE2));
    }
    ...
    public final float[] m5653fillBoundingBoxes8ffj60Q(final long range, final float[] array, int arrayStart) {
        requireIndexInRange(TextRange.m5786getMinimpl(range));
        requireIndexInRangeInclusiveEnd(TextRange.m5785getMaximpl(range));
        final Ref.IntRef intRef = new Ref.IntRef();
        intRef.element = arrayStart;
        final Ref.FloatRef floatRef = new Ref.FloatRef();
        MultiParagraphKt.m5660findParagraphsByRangeSbBc2M(this.paragraphInfoList, range, new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph$fillBoundingBoxes$1
            /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
            {
                super(1);
            }
            ...
            public final void invoke2(ParagraphInfo paragraphInfo) {
                ...
                long TextRange = TextRangeKt.TextRange(paragraphInfo.toLocalIndex(paragraphInfo.getStartIndex() > TextRange.m5786getMinimpl(j) ? paragraphInfo.getStartIndex() : TextRange.m5786getMinimpl(j)), paragraphInfo.toLocalIndex(paragraphInfo.getEndIndex() < TextRange.m5785getMaximpl(j) ? paragraphInfo.getEndIndex() : TextRange.m5785getMaximpl(j)));
                paragraphInfo.getParagraph().mo5619fillBoundingBoxes8ffj60Q(TextRange, fArr, intRef2.element);
                int m5784getLengthimpl = intRef2.element + (TextRange.m5784getLengthimpl(TextRange) * 4);
                for (int i = intRef2.element; i < m5784getLengthimpl; i += 4) {
                    int i2 = i + 1;
                    fArr[i2] = fArr[i2] + floatRef2.element;
                    int i3 = i + 3;
                    fArr[i3] = fArr[i3] + floatRef2.element;
                }
                intRef2.element = m5784getLengthimpl;
    ...
    public final long m5656getWordBoundaryjx7JFs(int offset) {
        int findParagraphByIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            findParagraphByIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(findParagraphByIndex);
        return paragraphInfo.m5668toGlobalxdX6G0(paragraphInfo.getParagraph().mo5623getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)), false);
    }
