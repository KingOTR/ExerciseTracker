...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m5786getMinimpl(j);
        this.selectionEnd = TextRange.m5785getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m5786getMinimpl = TextRange.m5786getMinimpl(j);
        int m5785getMaximpl = TextRange.m5785getMaximpl(j);
        if (m5786getMinimpl < 0 || m5786getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + m5786getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (m5785getMaximpl < 0 || m5785getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + m5785getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (m5786getMinimpl > m5785getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + m5786getMinimpl + " > " + m5785getMaximpl);
        }
    }
    ...
    public final TextRange m5947getCompositionMzsxiRA$ui_text_release() {
    ...
    public final long m5948getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long m5949updateRangeAfterDeletepWDy79M = EditingBufferKt.m5949updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        setSelectionStart(TextRange.m5786getMinimpl(m5949updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m5785getMaximpl(m5949updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long m5949updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m5949updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m5782getCollapsedimpl(m5949updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m5786getMinimpl(m5949updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m5785getMaximpl(m5949updateRangeAfterDeletepWDy79M2);
            }
        }
    }
