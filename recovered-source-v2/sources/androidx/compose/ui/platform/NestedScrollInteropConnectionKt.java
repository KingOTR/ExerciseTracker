...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m5566toOffsetUv8p0NA(int[] iArr, long j) {
        float coerceAtLeast;
        float coerceAtLeast2;
        if (Offset.m3602getXimpl(j) >= 0.0f) {
            coerceAtLeast = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m3602getXimpl(j));
        } else {
            coerceAtLeast = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m3602getXimpl(j));
        }
        if (Offset.m3603getYimpl(j) >= 0.0f) {
            coerceAtLeast2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m3603getYimpl(j));
        } else {
            coerceAtLeast2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m3603getYimpl(j));
        }
    ...
    public static final int m5567toViewTypeGyEprt8(int i) {
        return !NestedScrollSource.m4933equalsimpl0(i, NestedScrollSource.INSTANCE.m4945getUserInputWNlRxjI()) ? 1 : 0;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: getScrollAxes-k-4lQ0M, reason: not valid java name */
    public static final int m5565getScrollAxesk4lQ0M(long j) {
        int i = Math.abs(Offset.m3602getXimpl(j)) >= 0.5f ? 1 : 0;
        return Math.abs(Offset.m3603getYimpl(j)) >= 0.5f ? i | 2 : i;
    }
