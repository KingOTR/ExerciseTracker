...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver, RippleHostKey {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int i;
        this.rippleSize = contentDrawScope.mo4399getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m1421getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo4399getSizeNHjbRc()));
        } else {
            i = contentDrawScope.mo370roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long m3853unboximpl = this.color.getValue().m3853unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m1426drawStateLayerH2RKhps(contentDrawScope2, this.radius, m3853unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1425setRippleProperties07v42R4(contentDrawScope.mo4399getSizeNHjbRc(), m3853unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m1424addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m3853unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
