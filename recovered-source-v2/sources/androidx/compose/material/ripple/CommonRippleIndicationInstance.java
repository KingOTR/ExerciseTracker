...
final class CommonRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        if (Float.isNaN(this.radius)) {
            f = RippleAnimationKt.m1421getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo4399getSizeNHjbRc());
        } else {
            f = contentDrawScope.mo376toPx0680j_4(this.radius);
        }
        this.targetRadius = f;
        ...
        m1418drawRipples4WTKRHQ(contentDrawScope2, m3853unboximpl);
    }
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        RippleAnimation rippleAnimation = new RippleAnimation(this.bounded ? Offset.m3591boximpl(interaction.getPressPosition()) : null, this.targetRadius, this.bounded, null);
    ...
    private final void m1418drawRipples4WTKRHQ(DrawScope drawScope, long j) {
        Iterator<Map.Entry<PressInteraction.Press, RippleAnimation>> it = this.ripples.entrySet().iterator();
        while (it.hasNext()) {
            RippleAnimation value = it.next().getValue();
            float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
            if (pressedAlpha != 0.0f) {
                value.m1420draw4WTKRHQ(drawScope, Color.m3842copywmQWz5c$default(j, pressedAlpha, 0.0f, 0.0f, 0.0f, 14, null));
            }
        }
    }
