...
public abstract class RippleIndicationInstance implements IndicationInstance {
    ...
    public final void m1426drawStateLayerH2RKhps(DrawScope drawScope, float f, long j) {
        float f2;
        StateLayer stateLayer = this.stateLayer;
        if (Float.isNaN(f)) {
            f2 = RippleAnimationKt.m1421getRippleEndRadiuscSwnlzA(drawScope, this.bounded, drawScope.mo4399getSizeNHjbRc());
        } else {
            f2 = drawScope.mo376toPx0680j_4(f);
        }
        stateLayer.m1434drawStateLayermxwnekA(drawScope, f2, j);
    }
}
