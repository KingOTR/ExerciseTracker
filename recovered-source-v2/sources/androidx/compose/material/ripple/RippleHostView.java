...
public final class RippleHostView extends View {
    ...
    public final void m1424addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m1425setRippleProperties07v42R4(size, color, alpha);
        if (bounded) {
            unprojectedRipple.setHotspot(Offset.m3602getXimpl(interaction.getPressPosition()), Offset.m3603getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m1425setRippleProperties07v42R4(long size, long color, float alpha) {
        UnprojectedRipple unprojectedRipple = this.ripple;
        if (unprojectedRipple == null) {
            return;
        }
        unprojectedRipple.m1436setColorDxMtmZc(color, alpha);
        Rect rect = new Rect(0, 0, MathKt.roundToInt(Size.m3671getWidthimpl(size)), MathKt.roundToInt(Size.m3668getHeightimpl(size)));
        setLeft(rect.left);
        setTop(rect.top);
        setRight(rect.right);
