...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl mo330createnHHXs2Y(View view, boolean useTextDefault, long size, float cornerRadius, float elevation, boolean clippingEnabled, Density density, float initialZoom) {
        if (useTextDefault) {
            return new PlatformMagnifierImpl(new Magnifier(view));
        }
        long mo377toSizeXkaWNTQ = density.mo377toSizeXkaWNTQ(size);
        float mo376toPx0680j_4 = density.mo376toPx0680j_4(cornerRadius);
        float mo376toPx0680j_42 = density.mo376toPx0680j_4(elevation);
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (mo377toSizeXkaWNTQ != InlineClassHelperKt.UnspecifiedPackedFloats) {
            builder.setSize(MathKt.roundToInt(Size.m3671getWidthimpl(mo377toSizeXkaWNTQ)), MathKt.roundToInt(Size.m3668getHeightimpl(mo377toSizeXkaWNTQ)));
        }
        if (!Float.isNaN(mo376toPx0680j_4)) {
            builder.setCornerRadius(mo376toPx0680j_4);
        }
        if (!Float.isNaN(mo376toPx0680j_42)) {
            builder.setElevation(mo376toPx0680j_42);
        }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo329updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m3621isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m3602getXimpl(sourceCenter), Offset.m3603getYimpl(sourceCenter), Offset.m3602getXimpl(magnifierCenter), Offset.m3603getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m3602getXimpl(sourceCenter), Offset.m3603getYimpl(sourceCenter));
            }
        }
    }
}
