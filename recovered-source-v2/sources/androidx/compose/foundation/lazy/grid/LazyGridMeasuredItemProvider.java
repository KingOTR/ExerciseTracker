...
public abstract class LazyGridMeasuredItemProvider implements LazyLayoutMeasuredItemProvider<LazyGridMeasuredItem> {
    ...
    public abstract LazyGridMeasuredItem mo823createItemO3s9Psw(int index, Object key, Object contentType, int crossAxisSize, int mainAxisSpacing, List<? extends Placeable> placeables, long constraints, int lane, int span);
    ...
    public LazyGridMeasuredItem mo798getAndMeasurehBUhpc(int index, int lane, int span, long constraints) {
        return m828getAndMeasurem8Kt_7k(index, constraints, lane, span, this.defaultMainAxisSpacing);
    }

    /* renamed from: getAndMeasure-m8Kt_7k, reason: not valid java name */
    public final LazyGridMeasuredItem m828getAndMeasurem8Kt_7k(int index, long constraints, int lane, int span, int mainAxisSpacing) {
        ...
        List<Placeable> mo855measure0kLqBqw = this.measureScope.mo855measure0kLqBqw(index, constraints);
        if (Constraints.m6254getHasFixedWidthimpl(constraints)) {
            m6257getMinHeightimpl = Constraints.m6258getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m6253getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("does not have fixed height".toString());
            }
            m6257getMinHeightimpl = Constraints.m6257getMinHeightimpl(constraints);
        }
        return mo823createItemO3s9Psw(index, key, contentType, m6257getMinHeightimpl, mainAxisSpacing, mo855measure0kLqBqw, constraints, lane, span);
    }
