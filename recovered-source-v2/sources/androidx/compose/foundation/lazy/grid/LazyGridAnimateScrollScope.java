...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public float calculateDistanceTo(int targetIndex) {
        ...
        if (layoutInfo.getOrientation() == Orientation.Vertical) {
            m6431getXimpl = IntOffset.m6432getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            m6431getXimpl = IntOffset.m6431getXimpl(lazyGridItemInfo2.getOffset());
        }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (intValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == intValue) {
                    if (z) {
                        m6473getWidthimpl = IntSize.m6472getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        m6473getWidthimpl = IntSize.m6473getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    i4 = Math.max(i4, m6473getWidthimpl);
                    i++;
                }
                i2 += i4;
                i3++;
