...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        ellipsisInfo$foundation_layout_release.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m31getFirstimpl(ellipsisInfo$foundation_layout_release.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
        return ellipsisInfo$foundation_layout_release;
    }

    /* renamed from: getWrapInfo-OpUlnko, reason: not valid java name */
    public final WrapInfo m607getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        int i = totalCrossAxisSize + currentLineCrossAxisSize;
        if (nextSize == null) {
            return new WrapInfo(true, true);
        }
        if (this.overflow.getType$foundation_layout_release() != FlowLayoutOverflow.OverflowType.Visible && (lineIndex >= this.maxLines || IntIntPair.m32getSecondimpl(leftOver) - IntIntPair.m32getSecondimpl(nextSize.getPackedValue()) < 0)) {
            return new WrapInfo(true, true);
        }
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m31getFirstimpl(leftOver) - IntIntPair.m31getFirstimpl(nextSize.getPackedValue()) < 0)) {
            ...
            return new WrapInfo(true, m607getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m27constructorimpl(Constraints.m6256getMaxWidthimpl(this.constraints), (IntIntPair.m32getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m24boximpl(IntIntPair.m27constructorimpl(IntIntPair.m31getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m32getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int max = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m32getSecondimpl(nextSize.getPackedValue()));
        IntIntPair m612ellipsisSizeF35zmw$foundation_layout_release = isEllipsisWrap ? null : this.overflow.m612ellipsisSizeF35zmw$foundation_layout_release(nextItemHasNext, lineIndex, max);
        if (m612ellipsisSizeF35zmw$foundation_layout_release != null) {
            m612ellipsisSizeF35zmw$foundation_layout_release.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m31getFirstimpl(leftOver) - IntIntPair.m31getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m31getFirstimpl(m612ellipsisSizeF35zmw$foundation_layout_release.getPackedValue()) < 0) {
                if (isEllipsisWrap) {
                    return new WrapInfo(true, true);
                }
                WrapInfo m607getWrapInfoOpUlnko = m607getWrapInfoOpUlnko(false, 0, IntIntPair.m27constructorimpl(Constraints.m6256getMaxWidthimpl(this.constraints), (IntIntPair.m32getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m32getSecondimpl(nextSize.getPackedValue()))), m612ellipsisSizeF35zmw$foundation_layout_release, lineIndex + 1, max, 0, true, true);
                return new WrapInfo(m607getWrapInfoOpUlnko.getIsLastItemInContainer(), m607getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
