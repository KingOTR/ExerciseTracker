...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (textLayoutResult.getDidOverflowHeight() && !textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                int coerceAtMost = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m6472getHeightimpl(textLayoutResult.getSize())), textLayoutResult.getLineCount() - 1);
                while (coerceAtMost >= 0 && textLayoutResult.getLineTop(coerceAtMost) >= IntSize.m6472getHeightimpl(textLayoutResult.getSize())) {
                    coerceAtMost--;
                }
    ...
    public void appendSelectableInfoToBuilder(SelectionLayoutBuilder builder) {
        ...
        long m3606minusMKHz9U;
        LayoutCoordinates layoutCoordinates = getLayoutCoordinates();
        if (layoutCoordinates == null || (invoke = this.layoutResultCallback.invoke()) == null) {
            return;
        }
        long mo5188localPositionOfR5De75A = builder.getContainerCoordinates().mo5188localPositionOfR5De75A(layoutCoordinates, Offset.INSTANCE.m3618getZeroF1C5BW0());
        long m3606minusMKHz9U2 = Offset.m3606minusMKHz9U(builder.getCurrentPosition(), mo5188localPositionOfR5De75A);
        if (OffsetKt.m3623isUnspecifiedk4lQ0M(builder.getPreviousHandlePosition())) {
            m3606minusMKHz9U = Offset.INSTANCE.m3617getUnspecifiedF1C5BW0();
        } else {
            m3606minusMKHz9U = Offset.m3606minusMKHz9U(builder.getPreviousHandlePosition(), mo5188localPositionOfR5De75A);
        }
        MultiWidgetSelectionDelegateKt.m1333appendSelectableInfoParwq6A(builder, invoke, m3606minusMKHz9U2, m3606minusMKHz9U, getSelectableId());
    }
    ...
    public long mo1331getHandlePositiondBAh8RU(Selection selection, boolean isStartHandle) {
        TextLayoutResult invoke;
    ...
    public long mo1332getRangeOfLineContainingjx7JFs(int offset) {
        TextLayoutResult invoke = this.layoutResultCallback.invoke();
        if (invoke == null) {
