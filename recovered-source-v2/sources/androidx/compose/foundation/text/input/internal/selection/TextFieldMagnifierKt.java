...
public final class TextFieldMagnifierKt {
    ...
    public static final long m1248calculateSelectionMagnifierCenterAndroidhUlJWOE(TransformedTextFieldState transformedTextFieldState, TextFieldSelectionState textFieldSelectionState, TextLayoutState textLayoutState, long j) {
        int m5788getStartimpl;
        long m1272getHandleDragPositionF1C5BW0 = textFieldSelectionState.m1272getHandleDragPositionF1C5BW0();
        if (OffsetKt.m3623isUnspecifiedk4lQ0M(m1272getHandleDragPositionF1C5BW0) || transformedTextFieldState.getVisualText().length() == 0) {
        ...
        if (i == 1 || i == 2) {
            m5788getStartimpl = TextRange.m5788getStartimpl(selection);
        } else {
            if (i != 3) {
                throw new NoWhenBranchMatchedException();
            }
            m5788getStartimpl = TextRange.m5783getEndimpl(selection);
        }
        TextLayoutResult layoutResult = textLayoutState.getLayoutResult();
        if (layoutResult == null) {
            return Offset.INSTANCE.m3617getUnspecifiedF1C5BW0();
        }
        float m3602getXimpl = Offset.m3602getXimpl(m1272getHandleDragPositionF1C5BW0);
        int lineForOffset = layoutResult.getLineForOffset(m5788getStartimpl);
        float lineLeft = layoutResult.getLineLeft(lineForOffset);
        float lineRight = layoutResult.getLineRight(lineForOffset);
        float coerceIn = RangesKt.coerceIn(m3602getXimpl, Math.min(lineLeft, lineRight), Math.max(lineLeft, lineRight));
        ...
        if (textLayoutNodeCoordinates != null) {
            ...
            if (textLayoutNodeCoordinates != null) {
                Offset = TextLayoutStateKt.m1223coerceIn3MmeM6k(Offset, SelectionManagerKt.visibleBounds(textLayoutNodeCoordinates));
            }
        }
        return TextLayoutStateKt.m1225fromTextLayoutToCoreUv8p0NA(textLayoutState, Offset);
    }
}
