...
public final class TextFieldTextLayoutModifierNode extends Modifier.Node implements LayoutModifierNode, GlobalPositionAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    public MeasureResult mo94measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        TextLayoutResult m1221layoutWithNewMeasureInputshBUhpc = this.textLayoutState.m1221layoutWithNewMeasureInputshBUhpc(measureScope, measureScope.getLayoutDirection(), (FontFamily.Resolver) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, CompositionLocalsKt.getLocalFontFamilyResolver()), j);
        final Placeable mo5179measureBRTryo0 = measurable.mo5179measureBRTryo0(Constraints.INSTANCE.m6265fitPrioritizingWidthZbe2FdA(IntSize.m6473getWidthimpl(m1221layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m6473getWidthimpl(m1221layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m6472getHeightimpl(m1221layoutWithNewMeasureInputshBUhpc.getSize()), IntSize.m6472getHeightimpl(m1221layoutWithNewMeasureInputshBUhpc.getSize())));
        TextLayoutState textLayoutState = this.textLayoutState;
        if (this.singleLine) {
            m6303constructorimpl = measureScope.mo373toDpu2uoSUM(TextDelegateKt.ceilToIntPx(m1221layoutWithNewMeasureInputshBUhpc.getLineBottom(0)));
        } else {
            m6303constructorimpl = Dp.m6303constructorimpl(0);
        }
        textLayoutState.m1222setMinHeightForSingleLineField0680j_4(m6303constructorimpl);
        LinkedHashMap linkedHashMap = this.baselineCache;
        if (linkedHashMap == null) {
            linkedHashMap = new LinkedHashMap(2);
        }
        linkedHashMap.put(AlignmentLineKt.getFirstBaseline(), Integer.valueOf(Math.round(m1221layoutWithNewMeasureInputshBUhpc.getFirstBaseline())));
        linkedHashMap.put(AlignmentLineKt.getLastBaseline(), Integer.valueOf(Math.round(m1221layoutWithNewMeasureInputshBUhpc.getLastBaseline())));
        this.baselineCache = linkedHashMap;
        int m6473getWidthimpl = IntSize.m6473getWidthimpl(m1221layoutWithNewMeasureInputshBUhpc.getSize());
        int m6472getHeightimpl = IntSize.m6472getHeightimpl(m1221layoutWithNewMeasureInputshBUhpc.getSize());
        Map<AlignmentLine, Integer> map = this.baselineCache;
        Intrinsics.checkNotNull(map);
        return measureScope.layout(m6473getWidthimpl, m6472getHeightimpl, map, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldTextLayoutModifierNode$measure$1
            {
                super(1);
            }
