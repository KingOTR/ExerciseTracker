...
public final class TextFieldCoreModifierNode extends DelegatingNode implements LayoutModifierNode, DrawModifierNode, CompositionLocalConsumerModifierNode, GlobalPositionAwareModifierNode, SemanticsModifierNode {
    ...
    public MeasureResult mo94measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        if (this.orientation == Orientation.Vertical) {
            return m1201measureVerticalScroll3p2s80s(measureScope, measurable, j);
        }
        return m1200measureHorizontalScroll3p2s80s(measureScope, measurable, j);
    }
    ...
    public void draw(ContentDrawScope contentDrawScope) {
        ...
        if (TextRange.m5782getCollapsedimpl(visualText.getSelection())) {
            ...
            if (visualText.shouldShowSelection()) {
                m1199drawSelectionSbBc2M(contentDrawScope, visualText.getSelection(), layoutResult);
            }
    ...
    private final MeasureResult m1201measureVerticalScroll3p2s80s(final MeasureScope measureScope, Measurable measurable, long j) {
        final Placeable mo5179measureBRTryo0 = measurable.mo5179measureBRTryo0(Constraints.m6247copyZbe2FdA$default(j, 0, 0, 0, Integer.MAX_VALUE, 7, null));
        final int min = Math.min(mo5179measureBRTryo0.getHeight(), Constraints.m6255getMaxHeightimpl(j));
        return MeasureScope.layout$default(measureScope, mo5179measureBRTryo0.getWidth(), min, null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldCoreModifierNode$measureVerticalScroll$1
            ...
            public final void invoke2(Placeable.PlacementScope placementScope) {
                ...
                textFieldCoreModifierNode.m1202updateScrollStatetIlFzwE(measureScope2, i, height, transformedTextFieldState.getVisualText().getSelection(), measureScope.getLayoutDirection());
                Placeable placeable = mo5179measureBRTryo0;
    ...
    private final MeasureResult m1200measureHorizontalScroll3p2s80s(final MeasureScope measureScope, Measurable measurable, long j) {
        final Placeable mo5179measureBRTryo0 = measurable.mo5179measureBRTryo0(Constraints.m6247copyZbe2FdA$default(j, 0, Integer.MAX_VALUE, 0, 0, 13, null));
        final int min = Math.min(mo5179measureBRTryo0.getWidth(), Constraints.m6256getMaxWidthimpl(j));
        return MeasureScope.layout$default(measureScope, min, mo5179measureBRTryo0.getHeight(), null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.text.input.internal.TextFieldCoreModifierNode$measureHorizontalScroll$1
            /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
            {
                super(1);
            }
            ...
            public final void invoke2(Placeable.PlacementScope placementScope) {
                ...
                transformedTextFieldState = TextFieldCoreModifierNode.this.textFieldState;
                textFieldCoreModifierNode.m1202updateScrollStatetIlFzwE(measureScope2, i, width, transformedTextFieldState.getVisualText().getSelection(), measureScope.getLayoutDirection());
                Placeable placeable = mo5179measureBRTryo0;
                scrollState = TextFieldCoreModifierNode.this.scrollState;
    ...
    private final int m1198calculateOffsetToFollow72CqOWE(long currSelection, int currTextLayoutSize) {
        TextRange textRange = this.previousSelection;
        if (textRange == null || TextRange.m5783getEndimpl(currSelection) != TextRange.m5783getEndimpl(textRange.getPackedValue())) {
            return TextRange.m5783getEndimpl(currSelection);
        }
        TextRange textRange2 = this.previousSelection;
        if (textRange2 == null || TextRange.m5788getStartimpl(currSelection) != TextRange.m5788getStartimpl(textRange2.getPackedValue())) {
            return TextRange.m5788getStartimpl(currSelection);
        }
    ...
    public final void m1202updateScrollStatetIlFzwE(Density density, int i, int i2, long j, LayoutDirection layoutDirection) {
        TextLayoutResult layoutResult;
        Rect cursorRectInScroller;
        float f;
        this.scrollState.setMaxValue$foundation_release(i2 - i);
        int m1198calculateOffsetToFollow72CqOWE = m1198calculateOffsetToFollow72CqOWE(j, i2);
        if (m1198calculateOffsetToFollow72CqOWE < 0 || !getShowCursor() || (layoutResult = this.textLayoutState.getLayoutResult()) == null) {
            return;
        }
        Rect cursorRect = layoutResult.getCursorRect(RangesKt.coerceIn(m1198calculateOffsetToFollow72CqOWE, (ClosedRange<Integer>) new IntRange(0, layoutResult.getLayoutInput().getText().length())));
        ...
        if (bottom <= f2) {
            ...
            if (top >= f3 || bottom - top <= i) {
                ...
                this.previousSelection = TextRange.m5776boximpl(j);
    ...
    private final void m1199drawSelectionSbBc2M(DrawScope drawScope, long j, TextLayoutResult textLayoutResult) {
        int m5786getMinimpl = TextRange.m5786getMinimpl(j);
        int m5785getMaximpl = TextRange.m5785getMaximpl(j);
        if (m5786getMinimpl != m5785getMaximpl) {
            DrawScope.m4389drawPathLG529CI$default(drawScope, textLayoutResult.getPathForRange(m5786getMinimpl, m5785getMaximpl), ((TextSelectionColors) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, TextSelectionColorsKt.getLocalTextSelectionColors())).getBackgroundColor(), 0.0f, null, null, 0, 60, null);
        }
    }
    ...
    private final void drawHighlight(DrawScope drawScope, Pair<TextHighlightType, TextRange> pair, TextLayoutResult textLayoutResult) {
        ...
        if (TextRange.m5782getCollapsedimpl(packedValue)) {
            return;
        }
        Path pathForRange = textLayoutResult.getPathForRange(TextRange.m5786getMinimpl(packedValue), TextRange.m5785getMaximpl(packedValue));
        if (TextHighlightType.m1119equalsimpl0(value, TextHighlightType.INSTANCE.m1123getHandwritingDeletePreviewsxJuwY())) {
            Brush brush = textLayoutResult.getLayoutInput().getStyle().getBrush();
            if (brush != null) {
                DrawScope.m4388drawPathGBMwjPU$default(drawScope, pathForRange, brush, 0.2f, null, null, 0, 56, null);
                return;
            }
            long m5818getColor0d7_KjU = textLayoutResult.getLayoutInput().getStyle().m5818getColor0d7_KjU();
            if (m5818getColor0d7_KjU == 16) {
                m5818getColor0d7_KjU = Color.INSTANCE.m3869getBlack0d7_KjU();
            }
            long j = m5818getColor0d7_KjU;
            DrawScope.m4389drawPathLG529CI$default(drawScope, pathForRange, Color.m3842copywmQWz5c$default(j, Color.m3845getAlphaimpl(j) * 0.2f, 0.0f, 0.0f, 0.0f, 14, null), 0.0f, null, null, 0, 60, null);
            return;
        }
        DrawScope.m4389drawPathLG529CI$default(drawScope, pathForRange, ((TextSelectionColors) CompositionLocalConsumerModifierNodeKt.currentValueOf(this, TextSelectionColorsKt.getLocalTextSelectionColors())).getBackgroundColor(), 0.0f, null, null, 0, 60, null);
    }
    ...
    private final void drawCursor(DrawScope drawScope) {
        ...
        if (cursorAlpha != 0.0f && getShowCursor()) {
            ...
            DrawScope.m4384drawLine1RTmtNc$default(drawScope, this.cursorBrush, cursorRect.m3636getTopCenterF1C5BW0(), cursorRect.m3629getBottomCenterF1C5BW0(), cursorRect.getWidth(), 0, null, cursorAlpha, null, 0, 432, null);
        }
    }
