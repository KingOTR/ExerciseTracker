...
public final class PasswordInputTransformation implements InputTransformation {
    ...
    public void transformInput(TextFieldBuffer textFieldBuffer) {
        if (textFieldBuffer.getChanges().getChangeCount() != 1 || TextRange.m5784getLengthimpl(textFieldBuffer.getChanges().mo1109getRangejx7JFs(0)) != 1 || TextRange.m5784getLengthimpl(textFieldBuffer.getChanges().mo1108getOriginalRangejx7JFs(0)) != 0 || textFieldBuffer.hasSelection()) {
            setRevealCodepointIndex(-1);
            return;
        }
        int m5786getMinimpl = TextRange.m5786getMinimpl(textFieldBuffer.getChanges().mo1109getRangejx7JFs(0));
        if (getRevealCodepointIndex$foundation_release() != m5786getMinimpl) {
            this.scheduleHide.invoke();
            setRevealCodepointIndex(m5786getMinimpl);
        }
    }
