...
public final class TextFieldDelegate {
    ...
    public static final class Companion {
        ...
        public static /* synthetic */ Triple m1068layout_EkL_Y$foundation_release$default(Companion companion, TextDelegate textDelegate, long j, LayoutDirection layoutDirection, TextLayoutResult textLayoutResult, int i, Object obj) {
            if ((i & 8) != 0) {
                textLayoutResult = null;
            }
            return companion.m1071layout_EkL_Y$foundation_release(textDelegate, j, layoutDirection, textLayoutResult);
        }

        @JvmStatic
        /* renamed from: layout-_EkL_-Y$foundation_release, reason: not valid java name */
        public final Triple<Integer, Integer, TextLayoutResult> m1071layout_EkL_Y$foundation_release(TextDelegate textDelegate, long constraints, LayoutDirection layoutDirection, TextLayoutResult prevResultText) {
            TextLayoutResult m1058layoutNN6EwU = textDelegate.m1058layoutNN6EwU(constraints, layoutDirection, prevResultText);
            return new Triple<>(Integer.valueOf(IntSize.m6473getWidthimpl(m1058layoutNN6EwU.getSize())), Integer.valueOf(IntSize.m6472getHeightimpl(m1058layoutNN6EwU.getSize())), m1058layoutNN6EwU);
        }

        @JvmStatic
        /* renamed from: draw-Q1vqE60$foundation_release, reason: not valid java name */
        public final void m1070drawQ1vqE60$foundation_release(Canvas canvas, TextFieldValue value, long selectionPreviewHighlightRange, long deletionPreviewHighlightRange, OffsetMapping offsetMapping, TextLayoutResult textLayoutResult, Paint highlightPaint, long selectionBackgroundColor) {
            if (!TextRange.m5782getCollapsedimpl(selectionPreviewHighlightRange)) {
                highlightPaint.mo3722setColor8_81llA(selectionBackgroundColor);
                m1067drawHighlightLepunE(canvas, selectionPreviewHighlightRange, offsetMapping, textLayoutResult, highlightPaint);
            } else if (!TextRange.m5782getCollapsedimpl(deletionPreviewHighlightRange)) {
                Color m3833boximpl = Color.m3833boximpl(textLayoutResult.getLayoutInput().getStyle().m5818getColor0d7_KjU());
                if (m3833boximpl.m3853unboximpl() == 16) {
                    m3833boximpl = null;
                }
                long m3853unboximpl = m3833boximpl != null ? m3833boximpl.m3853unboximpl() : Color.INSTANCE.m3869getBlack0d7_KjU();
                highlightPaint.mo3722setColor8_81llA(Color.m3842copywmQWz5c$default(m3853unboximpl, Color.m3845getAlphaimpl(m3853unboximpl) * 0.2f, 0.0f, 0.0f, 0.0f, 14, null));
                m1067drawHighlightLepunE(canvas, deletionPreviewHighlightRange, offsetMapping, textLayoutResult, highlightPaint);
            } else if (!TextRange.m5782getCollapsedimpl(value.getSelection())) {
                highlightPaint.mo3722setColor8_81llA(selectionBackgroundColor);
                m1067drawHighlightLepunE(canvas, value.getSelection(), offsetMapping, textLayoutResult, highlightPaint);
            }
            TextPainter.INSTANCE.paint(canvas, textLayoutResult);
        }

        /* renamed from: drawHighlight-Le-punE, reason: not valid java name */
        private final void m1067drawHighlightLepunE(Canvas canvas, long range, OffsetMapping offsetMapping, TextLayoutResult textLayoutResult, Paint paint) {
            int originalToTransformed = offsetMapping.originalToTransformed(TextRange.m5786getMinimpl(range));
            int originalToTransformed2 = offsetMapping.originalToTransformed(TextRange.m5785getMaximpl(range));
        ...
        public final void notifyFocusedRect$foundation_release(TextFieldValue value, TextDelegate textDelegate, TextLayoutResult textLayoutResult, LayoutCoordinates layoutCoordinates, TextInputSession textInputSession, boolean hasFocus, OffsetMapping offsetMapping) {
            Rect rect;
            if (hasFocus) {
                int originalToTransformed = offsetMapping.originalToTransformed(TextRange.m5785getMaximpl(value.getSelection()));
                ...
                long mo5190localToRootMKHz9U = layoutCoordinates.mo5190localToRootMKHz9U(OffsetKt.Offset(rect.getLeft(), rect.getTop()));
                textInputSession.notifyFocusedRect(RectKt.m3642Recttz77jQw(OffsetKt.Offset(Offset.m3602getXimpl(mo5190localToRootMKHz9U), Offset.m3603getYimpl(mo5190localToRootMKHz9U)), SizeKt.Size(rect.getWidth(), rect.getHeight())));
            }
        }
        ...
        public final void updateTextLayoutResult$foundation_release(TextInputSession textInputSession, TextFieldValue textFieldValue, OffsetMapping offsetMapping, TextLayoutResultProxy textLayoutResult) {
            ...
            textInputSession.updateTextLayoutResult(textFieldValue, offsetMapping, textLayoutResult.getValue(), new Function1<Matrix, Unit>() { // from class: androidx.compose.foundation.text.TextFieldDelegate$Companion$updateTextLayoutResult$1$1$1
                ...
                public /* bridge */ /* synthetic */ Unit invoke(Matrix matrix) {
                    m1073invoke58bKbWc(matrix.m4095unboximpl());
                    return Unit.INSTANCE;
                }

                /* renamed from: invoke-58bKbWc, reason: not valid java name */
                public final void m1073invoke58bKbWc(float[] fArr) {
                    if (LayoutCoordinates.this.isAttached()) {
                        LayoutCoordinatesKt.findRootCoordinates(LayoutCoordinates.this).mo5194transformFromEL8BTi8(LayoutCoordinates.this, fArr);
                    }
                }
        ...
        public final void m1072setCursorOffsetULxng0E$foundation_release(long position, TextLayoutResultProxy textLayoutResult, EditProcessor editProcessor, OffsetMapping offsetMapping, Function1<? super TextFieldValue, Unit> onValueChange) {
            onValueChange.invoke(TextFieldValue.m6030copy3r_uNRQ$default(editProcessor.toTextFieldValue(), (AnnotatedString) null, TextRangeKt.TextRange(offsetMapping.transformedToOriginal(TextLayoutResultProxy.m1093getOffsetForPosition3MmeM6k$default(textLayoutResult, position, false, 2, null))), (TextRange) null, 5, (Object) null));
        }
        ...
        public final void onBlur$foundation_release(TextInputSession textInputSession, EditProcessor editProcessor, Function1<? super TextFieldValue, Unit> onValueChange) {
            onValueChange.invoke(TextFieldValue.m6030copy3r_uNRQ$default(editProcessor.toTextFieldValue(), (AnnotatedString) null, 0L, (TextRange) null, 3, (Object) null));
            textInputSession.dispose();
        }

        /* renamed from: applyCompositionDecoration-72CqOWE, reason: not valid java name */
        public final TransformedText m1069applyCompositionDecoration72CqOWE(long compositionRange, TransformedText transformed) {
            int originalToTransformed = transformed.getOffsetMapping().originalToTransformed(TextRange.m5788getStartimpl(compositionRange));
            int originalToTransformed2 = transformed.getOffsetMapping().originalToTransformed(TextRange.m5783getEndimpl(compositionRange));
