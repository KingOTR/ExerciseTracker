...
final class ToggleableElement extends ModifierNodeElement<ToggleableNode> {
    ...
    public void update(ToggleableNode node) {
        node.m948updateQzZPfjk(this.value, this.interactionSource, this.indicationNodeFactory, this.enabled, this.role, this.onValueChange);
    }

    @Override // androidx.compose.ui.node.ModifierNodeElement
    public void inspectableProperties(InspectorInfo inspectorInfo) {
        inspectorInfo.setName("toggleable");
        inspectorInfo.getProperties().set(Values.VECTOR_MAP_VECTORS_KEY, inspectorInfo.getValue());
    ...
    public int hashCode() {
        ...
        Role role = this.role;
        return ((hashCode3 + (role != null ? Role.m5595hashCodeimpl(role.getValue()) : 0)) * 31) + this.onValueChange.hashCode();
    }
}
