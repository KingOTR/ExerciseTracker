...
final class DefaultPagerNestedScrollConnection implements NestedScrollConnection {
    ...
    public final long m916consumeOnOrientationQWom1Mo(long j, Orientation orientation) {
        if (orientation == Orientation.Vertical) {
            return Velocity.m6536copyOhffZ5M$default(j, 0.0f, 0.0f, 2, null);
        }
        return Velocity.m6536copyOhffZ5M$default(j, 0.0f, 0.0f, 1, null);
    }

    @Override // androidx.compose.ui.input.nestedscroll.NestedScrollConnection
    /* renamed from: onPreScroll-OzD1aCk */
    public long mo760onPreScrollOzD1aCk(long available, int source) {
        if (NestedScrollSource.m4933equalsimpl0(source, NestedScrollSource.INSTANCE.m4945getUserInputWNlRxjI()) && Math.abs(this.state.getCurrentPageOffsetFraction()) > 1.0E-6d) {
            ...
            float f = -this.state.dispatchRawDelta(-RangesKt.coerceIn(this.orientation == Orientation.Horizontal ? Offset.m3602getXimpl(available) : Offset.m3603getYimpl(available), currentPageOffsetFraction, pageSize));
            float m3602getXimpl = this.orientation == Orientation.Horizontal ? f : Offset.m3602getXimpl(available);
            if (this.orientation != Orientation.Vertical) {
                f = Offset.m3603getYimpl(available);
            }
            return Offset.m3595copydBAh8RU(available, m3602getXimpl, f);
        }
    ...
    public long mo486onPostScrollDzOQY0M(long consumed, long available, int source) {
        if (NestedScrollSource.m4933equalsimpl0(source, NestedScrollSource.INSTANCE.m4944getSideEffectWNlRxjI()) && m915mainAxisk4lQ0M(available) != 0.0f) {
    ...
    public Object mo485onPostFlingRZ2iAVY(long j, long j2, Continuation<? super Velocity> continuation) {
        return Velocity.m6531boximpl(m916consumeOnOrientationQWom1Mo(j2, this.orientation));
    }

    /* renamed from: mainAxis-k-4lQ0M, reason: not valid java name */
    private final float m915mainAxisk4lQ0M(long j) {
