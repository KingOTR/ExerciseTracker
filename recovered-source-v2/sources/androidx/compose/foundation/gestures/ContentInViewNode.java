...
public final class ContentInViewNode extends Modifier.Node implements BringIntoViewResponder, LayoutAwareModifierNode, CompositionLocalConsumerModifierNode {
    ...
    private long viewportSize = IntSize.INSTANCE.m6478getZeroYbymL2g();
    ...
    public Rect calculateRectForParent(Rect localRect) {
        if (IntSize.m6471equalsimpl0(this.viewportSize, IntSize.INSTANCE.m6478getZeroYbymL2g())) {
            throw new IllegalStateException("Expected BringIntoViewRequester to not be used before parents are placed.".toString());
        }
        return m400computeDestinationO0kMr_c(localRect, this.viewportSize);
    }
    ...
    public Object bringChildIntoView(Function0<Rect> function0, Continuation<? super Unit> continuation) {
        ...
        if (invoke == null || m402isMaxVisibleO0kMr_c$default(this, invoke, 0L, 1, null)) {
    ...
    public void mo366onRemeasuredozmzZPI(long size) {
        Rect focusedChildBounds;
        long j = this.viewportSize;
        this.viewportSize = size;
        if (m398compareToTemP2vQ(size, j) < 0 && (focusedChildBounds = getFocusedChildBounds()) != null) {
            Rect rect = this.focusedChildBoundsFromPreviousRemeasure;
            if (rect == null) {
                rect = focusedChildBounds;
            }
            if (!this.isAnimationRunning && !this.trackingFocusedChild && m401isMaxVisibleO0kMr_c(rect, j) && !m401isMaxVisibleO0kMr_c(focusedChildBounds, size)) {
    ...
    public final float calculateScrollDelta(BringIntoViewSpec bringIntoViewSpec) {
        ...
        long m6485toSizeozmzZPI = IntSizeKt.m6485toSizeozmzZPI(this.viewportSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return bringIntoViewSpec.calculateScrollDistance(findBringIntoViewRequest.getTop(), findBringIntoViewRequest.getBottom() - findBringIntoViewRequest.getTop(), Size.m3668getHeightimpl(m6485toSizeozmzZPI));
        }
        if (i == 2) {
            return bringIntoViewSpec.calculateScrollDistance(findBringIntoViewRequest.getLeft(), findBringIntoViewRequest.getRight() - findBringIntoViewRequest.getLeft(), Size.m3671getWidthimpl(m6485toSizeozmzZPI));
        }
    ...
    private final Rect findBringIntoViewRequest() {
        ...
        if (size > 0) {
            ...
            do {
                ...
                if (invoke != null) {
                    if (m399compareToiLBOSCw(invoke.m3635getSizeNHjbRc(), IntSizeKt.m6485toSizeozmzZPI(this.viewportSize)) > 0) {
    ...
    private final Rect m400computeDestinationO0kMr_c(Rect childBounds, long containerSize) {
        return childBounds.m3639translatek4lQ0M(Offset.m3611unaryMinusF1C5BW0(m403relocationOffsetBMxPBkI(childBounds, containerSize)));
    }

    /* renamed from: isMaxVisible-O0kMr_c$default, reason: not valid java name */
    static /* synthetic */ boolean m402isMaxVisibleO0kMr_c$default(ContentInViewNode contentInViewNode, Rect rect, long j, int i, Object obj) {
        if ((i & 1) != 0) {
            j = contentInViewNode.viewportSize;
        }
        return contentInViewNode.m401isMaxVisibleO0kMr_c(rect, j);
    }

    /* renamed from: isMaxVisible-O0kMr_c, reason: not valid java name */
    private final boolean m401isMaxVisibleO0kMr_c(Rect rect, long j) {
        long m403relocationOffsetBMxPBkI = m403relocationOffsetBMxPBkI(rect, j);
        return Math.abs(Offset.m3602getXimpl(m403relocationOffsetBMxPBkI)) <= 0.5f && Math.abs(Offset.m3603getYimpl(m403relocationOffsetBMxPBkI)) <= 0.5f;
    }

    /* renamed from: relocationOffset-BMxPBkI, reason: not valid java name */
    private final long m403relocationOffsetBMxPBkI(Rect childBounds, long containerSize) {
        long m6485toSizeozmzZPI = IntSizeKt.m6485toSizeozmzZPI(containerSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return OffsetKt.Offset(0.0f, requireBringIntoViewSpec().calculateScrollDistance(childBounds.getTop(), childBounds.getBottom() - childBounds.getTop(), Size.m3668getHeightimpl(m6485toSizeozmzZPI)));
        }
        if (i == 2) {
            return OffsetKt.Offset(requireBringIntoViewSpec().calculateScrollDistance(childBounds.getLeft(), childBounds.getRight() - childBounds.getLeft(), Size.m3671getWidthimpl(m6485toSizeozmzZPI)), 0.0f);
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-TemP2vQ, reason: not valid java name */
    private final int m398compareToTemP2vQ(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Intrinsics.compare(IntSize.m6472getHeightimpl(j), IntSize.m6472getHeightimpl(j2));
        }
        if (i == 2) {
            return Intrinsics.compare(IntSize.m6473getWidthimpl(j), IntSize.m6473getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-iLBOSCw, reason: not valid java name */
    private final int m399compareToiLBOSCw(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Float.compare(Size.m3668getHeightimpl(j), Size.m3668getHeightimpl(j2));
        }
        if (i == 2) {
            return Float.compare(Size.m3671getWidthimpl(j), Size.m3671getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }
