...
public final class PagerSnapLayoutInfoProviderKt {
    ...
    public static final float dragGestureDelta(PagerState pagerState) {
        if (pagerState.getLayoutInfo().getOrientation() == Orientation.Horizontal) {
            return Offset.m3602getXimpl(pagerState.m932getUpDownDifferenceF1C5BW0$foundation_release());
        }
        return Offset.m3603getYimpl(pagerState.m932getUpDownDifferenceF1C5BW0$foundation_release());
    }
    ...
    public static final float calculateFinalSnappingBound(PagerState pagerState, LayoutDirection layoutDirection, float f, float f2, float f3, float f4) {
        ...
        if (FinalSnappingItem.m523equalsimpl0(calculateFinalSnappingItem, FinalSnappingItem.INSTANCE.m527getClosestItembbeMdSM())) {
            if (Math.abs(f5) > f) {
                if (!z) {
                    return f3;
                }
        ...
        } else if (!FinalSnappingItem.m523equalsimpl0(calculateFinalSnappingItem, FinalSnappingItem.INSTANCE.m528getNextItembbeMdSM())) {
            if (FinalSnappingItem.m523equalsimpl0(calculateFinalSnappingItem, FinalSnappingItem.INSTANCE.m529getPreviousItembbeMdSM())) {
                return f3;
            }
            return 0.0f;
        }
