...
final class ThumbNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo94measure3p2s80s(MeasureScope measureScope, Measurable measurable, long j) {
        float thumbDiameter;
        boolean z = (measurable.maxIntrinsicHeight(Constraints.m6256getMaxWidthimpl(j)) == 0 || measurable.maxIntrinsicWidth(Constraints.m6255getMaxHeightimpl(j)) == 0) ? false : true;
        if (this.isPressed) {
            thumbDiameter = SwitchTokens.INSTANCE.m3202getPressedHandleWidthD9Ej5fM();
        } else if (z || this.checked) {
            thumbDiameter = SwitchKt.getThumbDiameter();
        } else {
            thumbDiameter = SwitchKt.getUncheckedThumbDiameter();
        }
        float f = measureScope.mo376toPx0680j_4(thumbDiameter);
        ...
        final Placeable mo5179measureBRTryo0 = measurable.mo5179measureBRTryo0(Constraints.INSTANCE.m6266fixedJhjzzOo(floatValue, floatValue));
        final float f2 = measureScope.mo376toPx0680j_4(Dp.m6303constructorimpl(Dp.m6303constructorimpl(SwitchKt.SwitchHeight - measureScope.mo372toDpu2uoSUM(f)) / 2.0f));
        float f3 = measureScope.mo376toPx0680j_4(Dp.m6303constructorimpl(Dp.m6303constructorimpl(SwitchKt.SwitchWidth - SwitchKt.getThumbDiameter()) - SwitchKt.ThumbPadding));
        boolean z2 = this.isPressed;
        if (z2 && this.checked) {
            f2 = f3 - measureScope.mo376toPx0680j_4(SwitchTokens.INSTANCE.m3208getTrackOutlineWidthD9Ej5fM());
        } else if (z2 && !this.checked) {
            f2 = measureScope.mo376toPx0680j_4(SwitchTokens.INSTANCE.m3208getTrackOutlineWidthD9Ej5fM());
