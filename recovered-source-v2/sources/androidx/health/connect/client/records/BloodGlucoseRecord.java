...
public final class BloodGlucoseRecord implements InstantaneousRecord {
    ...
    public BloodGlucoseRecord(Instant time, ZoneOffset zoneOffset, BloodGlucose level, int i, int i2, int i3, androidx.health.connect.client.records.metadata.Metadata metadata) {
        ...
        UtilsKt.requireNotLess(level, level.zero$connect_client_release(), "level");
        UtilsKt.requireNotMore(level, MAX_BLOOD_GLUCOSE_LEVEL, "level");
    }
    ...
    static {
        Map<String, Integer> mapOf = MapsKt.mapOf(TuplesKt.to(RelationToMeal.GENERAL, 1), TuplesKt.to(RelationToMeal.AFTER_MEAL, 4), TuplesKt.to(RelationToMeal.FASTING, 2), TuplesKt.to(RelationToMeal.BEFORE_MEAL, 3));
        RELATION_TO_MEAL_STRING_TO_INT_MAP = mapOf;
        RELATION_TO_MEAL_INT_TO_STRING_MAP = UtilsKt.reverse(mapOf);
        Map<String, Integer> mapOf2 = MapsKt.mapOf(TuplesKt.to(SpecimenSource.INTERSTITIAL_FLUID, 1), TuplesKt.to(SpecimenSource.CAPILLARY_BLOOD, 2), TuplesKt.to(SpecimenSource.PLASMA, 3), TuplesKt.to(SpecimenSource.TEARS, 5), TuplesKt.to(SpecimenSource.WHOLE_BLOOD, 6), TuplesKt.to(SpecimenSource.SERUM, 4));
        SPECIMEN_SOURCE_STRING_TO_INT_MAP = mapOf2;
        SPECIMEN_SOURCE_INT_TO_STRING_MAP = UtilsKt.reverse(mapOf2);
    }
