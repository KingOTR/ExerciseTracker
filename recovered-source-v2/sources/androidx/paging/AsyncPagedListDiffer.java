...
public class AsyncPagedListDiffer<T> {
    ...
    public void submitList(final PagedList<T> pagedList, final Runnable commitCallback) {
        ...
        this.config.getBackgroundThreadExecutor().execute(new Runnable() { // from class: androidx.paging.AsyncPagedListDiffer$$ExternalSyntheticLambda1
            ...
            public final void run() {
                AsyncPagedListDiffer.submitList$lambda$2(PagedList.this, pagedList5, this, i, pagedList, recordingCallback, commitCallback);
            }
        });
    }
    ...
    public final void latchPagedList$paging_runtime_release(PagedList<T> newList, PagedList<T> diffSnapshot, PlaceholderPaddedDiffResult diffResult, RecordingCallback recordingCallback, int lastAccessIndex, Runnable commitCallback) {
        ...
        if (!newList.isEmpty()) {
            newList.loadAround(RangesKt.coerceIn(PlaceholderPaddedListDiffHelperKt.transformAnchorIndex(pagedList.getPlaceholderPaddedList(), diffResult, diffSnapshot.getPlaceholderPaddedList(), lastAccessIndex), 0, newList.size() - 1));
        }
