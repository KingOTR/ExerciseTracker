    ...
    private final LoadState computeLoadTypeState(LoadType loadType) {
        ...
        if (i == 1) {
            if (WhenMappings.$EnumSwitchMapping$0[loadType.ordinal()] == 1) {
                complete$paging_common_release = LoadState.NotLoading.INSTANCE.getIncomplete$paging_common_release();
            } else {
                complete$paging_common_release = LoadState.NotLoading.INSTANCE.getComplete$paging_common_release();
            }
