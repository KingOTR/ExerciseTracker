...
final class HpkeContext {
    ...
    static HpkeContext createContext(byte[] encapsulatedKey, byte[] sharedSecret, HpkeKem kem, HpkeKdf kdf, HpkeAead aead, byte[] info) throws GeneralSecurityException {
        ...
        return new HpkeContext(encapsulatedKey, kdf.labeledExpand(labeledExtract, concat, DatabaseFileArchive.COLUMN_KEY, hpkeSuiteId, aead.getKeyLength()), kdf.labeledExpand(labeledExtract, concat, "base_nonce", hpkeSuiteId, aead.getNonceLength()), maxSequenceNumber(aead.getNonceLength()), aead);
    }
