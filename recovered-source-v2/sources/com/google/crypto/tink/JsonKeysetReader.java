...
public final class JsonKeysetReader implements KeysetReader {
    ...
    private Keyset keysetFromJson(JsonObject json) throws IOException {
        ...
        JsonArray asJsonArray = json.getAsJsonArray(DatabaseFileArchive.COLUMN_KEY);
    ...
    private KeyData keyDataFromJson(JsonObject json) {
        ...
        if (this.urlSafeBase64) {
            decode = Base64.urlSafeDecode(json.get(Values.VECTOR_MAP_VECTORS_KEY).getAsString());
        } else {
            decode = Base64.decode(json.get(Values.VECTOR_MAP_VECTORS_KEY).getAsString());
        }
    ...
    private static void validateKeyset(JsonObject json) {
        if (!json.has(DatabaseFileArchive.COLUMN_KEY) || json.getAsJsonArray(DatabaseFileArchive.COLUMN_KEY).size() == 0) {
    ...
    private static void validateKeyData(JsonObject json) {
        if (!json.has("typeUrl") || !json.has(Values.VECTOR_MAP_VECTORS_KEY) || !json.has("keyMaterialType")) {
