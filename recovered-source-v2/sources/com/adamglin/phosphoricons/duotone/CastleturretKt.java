package com.adamglin.phosphoricons.duotone;

import androidx.compose.ui.graphics.ColorKt;
import androidx.compose.ui.graphics.PathFillType;
import androidx.compose.ui.graphics.SolidColor;
import androidx.compose.ui.graphics.StrokeCap;
import androidx.compose.ui.graphics.StrokeJoin;
import androidx.compose.ui.graphics.vector.ImageVector;
import androidx.compose.ui.graphics.vector.PathBuilder;
import androidx.compose.ui.unit.Dp;
import com.adamglin.phosphoricons.DuotoneGroup;
import kotlin.Metadata;
import kotlin.jvm.internal.Intrinsics;

/* compiled from: Castleturret.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\b\u0004\"\u0015\u0010\u0000\u001a\u00020\u0001*\u00020\u00028F¢\u0006\u0006\u001a\u0004\b\u0003\u0010\u0004\"\u0010\u0010\u0005\u001a\u0004\u0018\u00010\u0001X\u0082\u000e¢\u0006\u0002\n\u0000¨\u0006\u0006"}, d2 = {"CastleTurret", "Landroidx/compose/ui/graphics/vector/ImageVector;", "Lcom/adamglin/phosphoricons/DuotoneGroup;", "getCastleTurret", "(Lcom/adamglin/phosphoricons/DuotoneGroup;)Landroidx/compose/ui/graphics/vector/ImageVector;", "_castleTurret", "phosphor_release"}, k = 2, mv = {2, 0, 0}, xi = 48)
/* loaded from: classes3.dex */
public final class CastleturretKt {
    private static ImageVector _castleTurret;

    public static final ImageVector getCastleTurret(DuotoneGroup duotoneGroup) {
        Intrinsics.checkNotNullParameter(duotoneGroup, "<this>");
        ImageVector imageVector = _castleTurret;
        if (imageVector != null) {
            Intrinsics.checkNotNull(imageVector);
            return imageVector;
        }
        float f = (float) 24.0d;
        ImageVector.Builder builder = new ImageVector.Builder("CastleTurret", Dp.m6303constructorimpl(f), Dp.m6303constructorimpl(f), 256.0f, 256.0f, 0L, 0, false, 224, null);
        SolidColor solidColor = new SolidColor(ColorKt.Color(4278190080L), null);
        int m4196getButtKaPHkGw = StrokeCap.INSTANCE.m4196getButtKaPHkGw();
        int m4207getMiterLxFBmk8 = StrokeJoin.INSTANCE.m4207getMiterLxFBmk8();
        int m4126getNonZeroRgk1Os = PathFillType.INSTANCE.m4126getNonZeroRgk1Os();
        PathBuilder pathBuilder = new PathBuilder();
        pathBuilder.moveTo(208.0f, 48.0f);
        pathBuilder.verticalLineTo(92.69f);
        pathBuilder.arcToRelative(8.0f, 8.0f, 0.0f, false, true, -2.34f, 5.65f);
        pathBuilder.lineTo(192.0f, 112.0f);
        pathBuilder.verticalLineTo(224.0f);
        pathBuilder.horizontalLineTo(152.0f);
        pathBuilder.verticalLineTo(168.0f);
        pathBuilder.arcToRelative(24.0f, 24.0f, 0.0f, false, false, -48.0f, 0.0f);
        pathBuilder.verticalLineToRelative(56.0f);
        pathBuilder.horizontalLineTo(64.0f);
        pathBuilder.verticalLineTo(112.0f);
        pathBuilder.lineTo(50.34f, 98.34f);
        pathBuilder.arcTo(8.0f, 8.0f, 0.0f, false, true, 48.0f, 92.69f);
        pathBuilder.verticalLineTo(48.0f);
        pathBuilder.arcToRelative(8.0f, 8.0f, 0.0f, false, true, 8.0f, -8.0f);
        pathBuilder.horizontalLineTo(80.0f);
        pathBuilder.verticalLineTo(72.0f);
        pathBuilder.horizontalLineToRelative(32.0f);
        pathBuilder.verticalLineTo(40.0f);
        pathBuilder.horizontalLineToRelative(32.0f);
        pathBuilder.verticalLineTo(72.0f);
        pathBuilder.horizontalLineToRelative(32.0f);
        pathBuilder.verticalLineTo(40.0f);
        pathBuilder.horizontalLineToRelative(24.0f);
        pathBuilder.arcTo(8.0f, 8.0f, 0.0f, false, true, 208.0f, 48.0f);
        pathBuilder.close();
        ImageVector.Builder.m4534addPathoIyEayM$default(builder, pathBuilder.getNodes(), m4126getNonZeroRgk1Os, "", solidColor, 0.2f, null, 0.2f, 0.0f, m4196getButtKaPHkGw, m4207getMiterLxFBmk8, 4.0f, 0.0f, 0.0f, 0.0f, 14336, null);
        SolidColor solidColor2 = new SolidColor(ColorKt.Color(4278190080L), null);
        int m4196getButtKaPHkGw2 = StrokeCap.INSTANCE.m4196getButtKaPHkGw();
        int m4207getMiterLxFBmk82 = StrokeJoin.INSTANCE.m4207getMiterLxFBmk8();
        int m4126getNonZeroRgk1Os2 = PathFillType.INSTANCE.m4126getNonZeroRgk1Os();
        PathBuilder pathBuilder2 = new PathBuilder();
        pathBuilder2.moveTo(216.0f, 216.0f);
        pathBuilder2.horizontalLineTo(200.0f);
        pathBuilder2.verticalLineTo(115.31f);
        pathBuilder2.lineTo(211.31f, 104.0f);
        pathBuilder2.arcTo(15.86f, 15.86f, 0.0f, false, false, 216.0f, 92.69f);
        pathBuilder2.verticalLineTo(48.0f);
        pathBuilder2.arcToRelative(16.0f, 16.0f, 0.0f, false, false, -16.0f, -16.0f);
        pathBuilder2.horizontalLineTo(176.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, -8.0f, 8.0f);
        pathBuilder2.verticalLineTo(64.0f);
        pathBuilder2.horizontalLineTo(152.0f);
        pathBuilder2.verticalLineTo(40.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, -8.0f, -8.0f);
        pathBuilder2.horizontalLineTo(112.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, -8.0f, 8.0f);
        pathBuilder2.verticalLineTo(64.0f);
        pathBuilder2.horizontalLineTo(88.0f);
        pathBuilder2.verticalLineTo(40.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, -8.0f, -8.0f);
        pathBuilder2.horizontalLineTo(56.0f);
        pathBuilder2.arcTo(16.0f, 16.0f, 0.0f, false, false, 40.0f, 48.0f);
        pathBuilder2.verticalLineTo(92.69f);
        pathBuilder2.arcTo(15.86f, 15.86f, 0.0f, false, false, 44.69f, 104.0f);
        pathBuilder2.lineTo(56.0f, 115.31f);
        pathBuilder2.verticalLineTo(216.0f);
        pathBuilder2.horizontalLineTo(40.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, 0.0f, 16.0f);
        pathBuilder2.horizontalLineTo(216.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, 0.0f, -16.0f);
        pathBuilder2.close();
        pathBuilder2.moveTo(69.66f, 106.34f);
        pathBuilder2.lineTo(56.0f, 92.69f);
        pathBuilder2.verticalLineTo(48.0f);
        pathBuilder2.horizontalLineTo(72.0f);
        pathBuilder2.verticalLineTo(72.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, 8.0f, 8.0f);
        pathBuilder2.horizontalLineToRelative(32.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, 8.0f, -8.0f);
        pathBuilder2.verticalLineTo(48.0f);
        pathBuilder2.horizontalLineToRelative(16.0f);
        pathBuilder2.verticalLineTo(72.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, 8.0f, 8.0f);
        pathBuilder2.horizontalLineToRelative(32.0f);
        pathBuilder2.arcToRelative(8.0f, 8.0f, 0.0f, false, false, 8.0f, -8.0f);
        pathBuilder2.verticalLineTo(48.0f);
        pathBuilder2.horizontalLineToRelative(16.0f);
        pathBuilder2.verticalLineTo(92.69f);
        pathBuilder2.lineToRelative(-13.66f, 13.65f);
        pathBuilder2.arcTo(8.0f, 8.0f, 0.0f, false, false, 184.0f, 112.0f);
        pathBuilder2.verticalLineTo(216.0f);
        pathBuilder2.horizontalLineTo(160.0f);
        pathBuilder2.verticalLineTo(168.0f);
        pathBuilder2.arcToRelative(32.0f, 32.0f, 0.0f, false, false, -64.0f, 0.0f);
        pathBuilder2.verticalLineToRelative(48.0f);
        pathBuilder2.horizontalLineTo(72.0f);
        pathBuilder2.verticalLineTo(112.0f);
        pathBuilder2.arcTo(8.0f, 8.0f, 0.0f, false, false, 69.66f, 106.34f);
        pathBuilder2.close();
        pathBuilder2.moveTo(144.0f, 216.0f);
        pathBuilder2.horizontalLineTo(112.0f);
        pathBuilder2.verticalLineTo(168.0f);
        pathBuilder2.arcToRelative(16.0f, 16.0f, 0.0f, false, true, 32.0f, 0.0f);
        pathBuilder2.close();
        ImageVector.Builder.m4534addPathoIyEayM$default(builder, pathBuilder2.getNodes(), m4126getNonZeroRgk1Os2, "", solidColor2, 1.0f, null, 1.0f, 0.0f, m4196getButtKaPHkGw2, m4207getMiterLxFBmk82, 4.0f, 0.0f, 0.0f, 0.0f, 14336, null);
        ImageVector build = builder.build();
        _castleTurret = build;
        Intrinsics.checkNotNull(build);
        return build;
    }
}
